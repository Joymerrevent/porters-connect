...
// The generic Read/Write flow is unit-tested in core/data-read.test.ts and
// core/data-write.test.ts; here we pin the
// Candidate-specific catalog (each field decodes by its Data Type) and config
...
describe("createCandidateResource — decode catalog", () => {
  it("decodes every catalogued field by its Data Type", async () => {
    ...
    // Custom/dynamic access by string isn't on the closed ReadRecord type (U1); the runtime
    // record still holds every alias, so read it through a loose view for the by-name loops.
