...
取得した Access Token は [HTTP ヘッダ](headers.md) の `X-porters-hrbc-oauth-token` に載せて Resource API を呼ぶ。
失効時は `grant_type=refresh_token`（`code` に Refresh Token）で更新。エラーは [認証エラー](errors.md)。
