...
ヘッダに Access Token。詳細は [authentication.md][ref1] / [resource-api.md][ref2]。
...
- **System[Reference] / User**: **ID のみ** `<FieldAlias>123</FieldAlias>`。
  - 参照先 ID の出どころに注意（例: `{Resource}.P_Candidate` は `Person.P_Id`）。詳細は [field-data-types.md][ref3]。
