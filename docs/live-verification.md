...
確認が取れたら、**コード側のコメントと下記エントリの両方を更新**（必要なら ADR へ昇格、fixture を実データへ差し替え）します。
...
| LV-5 | リソース毎の create 必須項目 | 確定 2026-06-17 |
...
- **状態**: 確定 2026-06-17 — reference で解決（Candidate=`P_Owner`／Job=+`P_Client`,`P_Recruiter`／Client=`P_Owner`／Process=関連6（`P_Client`/`P_Recruiter`/`P_Job`/`P_Candidate`/`P_Resume`）／Resume=+`P_Candidate`）。P_Id は System[Id]＝lib 供給で除外
