...
| LV-5 | リソース毎の create 必須項目 | `P_Owner` のみ型で必須化（ADR-0019 W2）。他は任意                   | `P_Owner` は「通常必須」止まり。Process/Resume は関連項目（`P_Candidate`/`P_Job` 等）も必須の可能性 | `src/resources/*.ts`（`REQUIRED_ON_CREATE`） | 実 create で必須エラーになる項目を確認     | 未確認         |
