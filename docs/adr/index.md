...
| [0049][0049] | `host` 書式検証で既定ポートを落とさない（RV-21）                   | 基本設計 | superseded by 0078 | 0.7.0  |
...
| [0055][0055] | `partition` が束ねられていない呼び出しの扱い（RV-25）              | 基本設計 | accepted           | 0.10.0 |
| [0056][0056] | `P_Deleted`（削除フラグ）の扱い（RV-26）                           | 詳細設計 | accepted           | 0.10.0 |
| [0057][0057] | `itemstate: "existing"` の明示指定を送るか（0038 SD-4 の amend）   | 詳細設計 | accepted           | 0.10.0 |
| [0058][0058] | `System[Reference]` を展開して読むときの扱い（RV-31）              | 基本設計 | accepted           | 0.11.0 |
| [0059][0059] | Read の `field` を接頭辞なしの alias で受ける                      | 基本設計 | accepted           | 0.11.0 |
| [0060][0060] | 次の主軸を「全リソース網羅 ＋ ドキュメント充実」に変える           | 要件定義 | accepted           | —      |
| [0061][0061] | Phase の公開サーフェス（接頭辞なし・resource 必須）                | 詳細設計 | accepted           | 0.12.0 |
| [0062][0062] | back-merge も PR を通す（ADR-0030 の実行方式を改訂）               | プロセス | accepted           | —      |
| [0063][0063] | 冪等性ガードを「送信済みで結果が不明」に限定する                   | 詳細設計 | accepted           | 0.12.1 |
| [0064][0064] | Link / Image 型の対応（宣言から Read / Write まで）                | 基本設計 | accepted           | 0.14.0 |
...
| [0069][0069] | テナントの Field カタログで宣言を突き合わせ、宣言を生成する        | 詳細設計 | accepted           | 0.15.0 |
...
| [0073][0073] | スロットルの共有単位（client ごとをやめ、注入できるようにする）    | 詳細設計 | accepted           | 0.15.0 |
| [0074][0074] | 未宣言のカスタム項目を `field` からも外す（宣言必須へ揃える）      | 詳細設計 | accepted           | 0.16.0 |
| [0075][0075] | Attachment に `searchAll` を足し、本体は `get` でだけ運ぶ          | 詳細設計 | accepted           | 0.17.0 |
| [0076][0076] | Phase の Read クエリから `keywords` / `itemstate` を外す           | 詳細設計 | accepted           | 0.17.0 |
| [0077][0077] | 既定 transport のタイムアウトを利用者が変えられるようにする        | 詳細設計 | accepted           | 0.17.0 |
| [0078][0078] | アクセスポイントを `hostname` と `port` に分ける                   | 基本設計 | accepted           | 0.18.0 |
| [0079][0079] | リソースの指定は数値のまま、名前との変換を公開する                 | 詳細設計 | accepted           | 0.18.0 |
| [0080][0080] | URL パラメータのリソースは `of()` で束ねる（0022 の Field を改訂） | 詳細設計 | accepted           | 0.18.0 |
| [0081][0081] | Attachment の Read を出典の語彙に合わせる（0018 を改訂）           | 詳細設計 | accepted           | 0.18.0 |
| [0082][0082] | CJS は `require` 条件で解決し、Node の下限を 22 に上げる           | 基本設計 | accepted           | 0.19.0 |
...
| [0102][0102] | スロットルを、どの 1 分間でも上限を超えない形にする                | 詳細設計 | proposed           | —      |
| [0103][0103] | `create` の Code 302 は自動で再送しない                            | 詳細設計 | proposed           | —      |
| [0104][0104] | 宣言できない項目の宣言で `verifyFields` の `ok` を倒す             | 詳細設計 | proposed           | —      |
| [0105][0105] | 検索の値の区切り文字は送る前に拒否する                             | 詳細設計 | proposed           | —      |
