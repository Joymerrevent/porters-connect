...
| [0049][0049] | `host` 書式検証で既定ポートを落とさない（RV-21）            | 基本設計 | proposed   |
| [0050][0050] | 認証 API 経路にも HTTP ステータスを配線（RV-19）            | 詳細設計 | proposed   |
...
- [0049][0049] `host` 書式検証で既定ポートを落とさない（RV-21）— **推奨は案C**（既定ポートを持たない
  スキームでパースし、`:443` も含め**どのポートも落とさない**）。[0048][0048] の**機構だけ**を差し替える改訂で、
  受け入れを広げるだけ＝semver は patch。**0.7.0 の前に決めるのが望ましい**
  （この検証が世に出るのが 0.7.0 なので、先に入れれば「壊してから戻す」ことにならない）
- [0050][0050] 認証 API 経路にも HTTP ステータスを配線（RV-19）— **推奨は案A**（[0044][0044] の `readResponse` を
  共有し、判定順・写像はそのまま認証 2 経路にも適用）。新しい決定は持ち込まず**適用範囲を広げるだけ**・semver は minor。
  **0.7.0 のブロッカーではない**（既存の挙動で、今回の変更による退行ではない）
...
- [0048][0048]（案A ＋ 機構2）アクセスポイント `host` の書式検証 — semver は patch。
  0047 の続き（`host` の意味を実行時にも強制する）。**機構2 の改訂を [0049][0049] で議論中**
