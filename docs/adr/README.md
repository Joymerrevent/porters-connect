...
| [0004][0004] | リソース／フィールドの型モデル（P\_ ＋ U\_/A\_） | accepted   |

## 論点バックログ（未起票）

**番号は付けない**（起票時に採番）。前方参照はトピック名で行う。

フェーズタグ：**【要件定義】**何を作るか（PRD が担当）／ **【基本設計】**外部仕様・全体像（公開 API・型・エラー・認証面・層責務）／ **【詳細設計】**内部実装（実装フェーズで決める）。
...
- **公開 API の形** — `PortersClient` オプション・リソースアクセサの返り値型・カスタム項目の宣言 DSL・第2層 MCP の接ぎ目
...
- **リトライ/スロットリングの機構** — 方針（フェイルセーフ・良き API 市民）は決定済み、実装方式は実装時
- **XML パース/シリアライズ内部** — データ型別の値エンコード（Option/参照/User/Image）・Read/Write 非対称
- **トークンストア内部** — 既定インメモリ＋永続化フック

### 決定済み（ADR / PRD）

- 型モデル: [ADR-0004][0004]／日時の表現: PRD R-10（ISO 8601・UTC）／MVP: [ADR-0003][0003]／接地方針: [ADR-0002][0002]
