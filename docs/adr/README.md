...
| 候補 # | 論点                                                                             |
| ------ | -------------------------------------------------------------------------------- |
| 0003   | HTTP トランスポート（標準 `fetch` か `ky` か）＋ リトライ/スロットリング         |
| 0004   | エラーモデル（判別可能 union の設計）                                            |
| 0005   | OAuth トークンのライフサイクル（`code` / `code_direct`・取得・キャッシュ・更新） |
| 0006   | XML → 型 の戦略（手書き型 + テスト担保 / スキーマ駆動）                          |
| 0007   | 公開 API の形（`PortersClient` のオプション・リソースアクセサの返り値型）        |
| 0008   | ページング・検索条件の抽象化                                                     |
| 0009   | 日時の UTC ↔ JST 取り扱い                                                        |
