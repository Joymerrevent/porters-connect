# 41. 一括書き込み面 `createMany` / `updateMany` の詳細設計（F-4）
...
> 求める一括書き込みを公開面に落とす。**内部（`buildWriteXml` の配列・`parseWriteResult` の per-item）は実装済み**で、
...
>
> **decider に諮る論点**: 軸1（`createMany`/`updateMany` 分離 か 混在 `writeMany` か）・軸2（部分成功の戻り値/エラー方針）・
> 軸4（チャンク失敗時の挙動）。下記は推奨案。
...
- **公開面は単件のみ**。`createResource` の `write(item, idempotent)` は `items:[item]` の**単一要素**で送り、
...
- **薄く・堅く・メンテしやすく**（`CLAUDE.md`）: 内部は既に配列/per-item 対応＝**公開面と分割ロジックの薄い追加**で足りる。型機構を過剰にしない。
...
推奨採用: **案1a ／ 案2a ／ 案3a ／ 案4a ／ Attachment 対象外 ／ 追加 minor ／ 軸7 テスト**。サブ決定:
