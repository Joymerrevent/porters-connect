...
- [ADR-0029][adr29] の back-merge＝案F は本 ADR で **superseded**（案H に改訂）。ADR-0029 へは status ポインタのみ追記し、決定本文（案F）は記録として残す。
- back-merge は **手動**（runbook §2 の `git merge` 1ステップ）。ワークフロー・Secret の追加なし＝完全トークンレスを維持。
- 実装 PR #76（`tag.yml` は back-merge を含めず・runbook は手動）はこの決定と整合。
