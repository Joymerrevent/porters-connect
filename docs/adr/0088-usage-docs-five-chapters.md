...
**主題別（`topics/`）**: 10 本。各ページは **まず知ること（PORTERS 側の前提）→ 使い方 → 細かい規則** の順で書き、
末尾に `## 関連`（検査⑤）。
...
2. **リソース別**: `resources/README.md` と 18 本を書く。`index.md` の表を消す
3. **主題別と実践例**: 統合・書き直し（上記「書き直す箇所」）。`concepts/` が消える
...
- **Bad**: 手書きのページが 20 → 約 38 本に増える（導入 6・主題 10・リソース 19・実践 2・troubleshooting 1）。
