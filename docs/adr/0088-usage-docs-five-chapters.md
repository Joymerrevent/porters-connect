...
> （「実装の分け方」の 4 本を順に）。
>
> **章の名前の変更（2026-09-25・stakeholder）**: 「主題別」を「ガイド」に、「リソース別」を「リソース」に改めた。
> 章の構成とディレクトリ（`topics/`・`resources/`）は変えていない。本文の決定の文面は書き換えない。
...
**訂正（実装時 2026-09-22・stakeholder）**: 目次は**最初のページ**なので、上から順に読んで学べる形にする。
冒頭に「このライブラリについて」（何をする・しない・何が要る）と「このドキュメントで学べること」（5 章で何が
できるようになるか・読む順）を置き、章は学ぶ順に番号を付け、「目的から探す」は**末尾の付録**にする。
目次にリソースの表のような内容を持たない、という趣旨は変わらない。

**訂正（実装後 2026-09-23・stakeholder）**: 5 章に**「クライアント」と「関数」の章を足し、7 章にする**（起票時は
~~「クライアントと関数」1 章・6 章~~ だったが、同日の実装中に stakeholder が関数を別の章に分け、リソース別の後に置いた）。
「クライアント」は主題別の後・リソース別の前（読者の順路が `porters` → `tenant(id)` → `t.candidate` の順なので、リソースの前に置く）、
「関数」はリソース別の後（順路の外にある道具なので、順路に沿った章の後に置く）。
...
stakeholder が指摘）。ページは `docs/usage/client/` に 3 本と `docs/usage/functions/` に 3 本:
...
| `functions/declare.md`   | 宣言と突合: `defineFields` / `generateFieldDecls` / `verifyFields` / `assertFieldsMatch` / `readCustomCatalog` / `rawValue`                                          |
...
目次の章番号は 1 導入 / 2 主題別 / 3 クライアント / 4 リソース別 / 5 関数 / 6 実践例 / 7 リファレンス になり、
手書きのページは約 39 → 45 本。~~実装は [#378][pr378] のマージ後・別 PR~~ **実装も [#378][pr378] に含めた**（同日・stakeholder。
訂正注記を付けた直後に同じブランチで 4 ページと検査の拡張を積んだ）。決定そのもの（主題・リソースで切る軸、
