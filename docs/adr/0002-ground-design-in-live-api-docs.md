...
- **P1**（認証・型システム・Write 形式・運用の落とし穴）と **P2**（レート挙動・Phase・仕様変更・開発環境）は
  **設計着手前に収集済み**。
- **P3** は再評価し個別対応とした（2026-06-13）: **用語集は収集**（`glossary.md`）。
  **Option Default List**（巨大・テナントで上書き可）と **Attachment Mime Type List**（v0.2 向け）は
  **転記せず出典リンクのみ**とする（実値は Option Read / 必要時に参照）。
...
- 用語集は `glossary.md` に収集済み。出典リンクのみ（転記しない）: Option - Default Option List（巨大・テナント依存）／ Attachment - Mime Type List（v0.2）。
