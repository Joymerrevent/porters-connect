...
- **P1**（認証・型システム・Write 形式・運用の落とし穴）と **P2**（レート挙動・Phase・仕様変更・開発環境）は
  **設計着手前に収集済み**。
- **P3** は再評価し個別対応とした（2026-06-13）: **用語集**（`glossary.md`）と
  **Attachment の Mime Type List**（小さく実用的・`resources/attachment.md` に付録）は**収集**。
  **Option Default List**（約1400行・テナントで上書き可）のみ **転記せず出典リンク**とする（実値は Option Read で取得）。
