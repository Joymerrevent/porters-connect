...
（第2層 MCP / リソース拡充 / オフライン評価基盤 / `defineFields` 深掘り / 採用面の地固め）。
...
推奨: **案A（第2層 MCP）を次の主軸**とし、その評価基盤として**案C の薄い版を先行/並行**、
リソースは**案B を MCP 露出の需要に応じて機会的に追加**。案D / 案E は後続。
...
- 関連: [[0024-mock-transport]]（案C の前提）、[[0023-custom-field-declaration-dsl]]（案D）、
