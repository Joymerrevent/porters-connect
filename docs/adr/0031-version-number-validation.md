...
**推奨（proposed・チーム決定待ち。accept は議論後に反映）：**
...
「CI を脆くしない」「薄く・robust（フェイルセーフ）」方針に反するため非推奨。

### Consequences

- （accept 後に記入）`scripts/check-release-invariants.mjs` に形式検証・単調増加検証を追加。ci.yml の checkout に tags 取得を追加。
- 実装は**別 PR**（accept 後）。
