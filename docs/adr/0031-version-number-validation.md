...
> **［2026-06-22 追記・supersede］** 単調増加検証(2)の**実行範囲**は [ADR-0032][adr32] で
> **base=main の PR に限定**へ改訂（毎 PR → リリース PR のみ）。git-flow の手動 back-merge ラグで
> develop の無関係 PR を「版の逆行」で誤検知したため（project-review RV-9）。形式検証(1)・baseline・
> 自前比較は不変。以下の決定本文は当時の記録として残す。
