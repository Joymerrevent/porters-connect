...
`contact` / `opportunity` / `process` / `resume`）です。マスタ系と Attachment はカスタム項目を持たないため受け付けません
...
- **リソース名が既知であること** — `candidate` / `job` / `client` / `recruiter` / `contact` / `opportunity` / `process` / `resume` のみ。
