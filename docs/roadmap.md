...
D1〜D5 と同じく**検査できる形**に落とすと 4 つになる。

| ID  | 条件                                   | 何で測るか                                                                                      | 現在地                           |
| --- | -------------------------------------- | ----------------------------------------------------------------------------------------------- | -------------------------------- |
| V1  | **自分で進められる作業が残っていない** | 本書の「着手可能」「判断待ち」「要 ADR」がすべて 0 件 ／ [findings][findings] の open が 0 件   | 着手可能 2・判断待ち 3・要 ADR 1 |
| V2  | **残る未確認はライブ検証だけ**         | `grep -rn "VERIFY(live)" src test` の結果が [live-verification][lv] のエントリと 1:1 で対応する | LV-1〜22（22 件）                |
| V3  | **契約後、その検証を全件確定させる**   | [live-verification][lv] の全エントリが「状態: **確定**」                                        | **0 / 22**（契約待ち）           |
| V4  | **検証で外れた仮定の修正が完了**       | V3 で判明した差分への対応（必要なら ADR ＋ 実装）が入っている                                   | — （V3 の後）                    |

> ⚠️ **この定義は「`1.0.0` は契約なしには出せない」を意味する**（V3 が契約待ちのため）。
...
- [ ] **v1 で CJS 出力まで出すか**（PRD [§8][prd]・[eng]）— **V1 の対象**（判断待ちが 0 になるまで
...
| 待ちの種類   | 中身                                                                                                |
| ------------ | --------------------------------------------------------------------------------------------------- |
| **契約待ち** | ライブ検証 **LV-1〜22**（[live-verification][lv]）。0.x のブロッカーではないが **`1.0.0` の V3**    |
| **需要待ち** | フェイクサーバー **フェーズ7**（package 昇格・配布。[実装計画][fake-plan]・stakeholder 2026-08-09） |
| **判断待ち** | 上記「判断待ち」節を参照（CJS 出力・Sales の必須・成功指標の数値化）＝ **V1 の対象**                |
...
| ----------------------------- | -------------------------------------------------- | ---------------------------------------- |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 2・判断待ち 3・要 ADR 1（＝V1） |
