...
- [ ] 4. **Activity**（`P_Resource` ＋ `P_ResourceId` の可変参照が論点）→ 5. **Contract**（**`P_Owner` が無い**・40 項目）→ 6. **Sales**（参照 6 項目の必須が `※`＝条件付き。要判断）
...
「どこまで完成しているか」の実測（2026-08-30 に再実測。初出は 2026-08-16 の[レビュー][run816]）。
**この 5 行が [ADR-0060][adr60] の完了条件 D1〜D5** ＝「網羅した」を主張でなく**検査結果**で言えるようにしたもの。

| ID  | 軸                         | 現在地                    | 目標と残り                                                                                                                        |
| --- | -------------------------- | ------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| D1  | **データ系リソースの R/W** | **6 / 13**                | → 13/13。未実装は Recruiter / Contact / Activity / Contract / Sales / Opportunity / Phase                                         |
| D2  | **マスタ Read の標準項目** | User 4/17 ほか            | → 全項目。Field 9/10（`P_ResourceType` 欠）・Option 6/7（`Items` 欠）・Partition 3/3 済。「マスタ 4/4」は**リソース単位**の数え方 |
| D3  | **データ型網羅**           | **14 / 17**               | → 17/17。`Link` / `Image`（R-4・カスタム項目経由でのみ出現）と `System[Department]`（User / Phase・LV 待ちの部分あり）            |
| D4  | reference ↔ カタログ突合   | データ系 5 種             | → 全リソース（`test/integration/reference-catalog.test.ts`。RV-29 の仕組みを広げる）                                              |
| D5  | **ドキュメント**           | README ＋ **ガイド 6 本** | → 全リソース表 ＋ 新規トピックのガイド（[ADR-0035][adr35] の型）                                                                  |

**API 機能網羅は別枠で実質完成** ✅ — Read 8 パラメータ・Write・一括・ページング・OAuth 2 方式・
マルチテナント・制限対応・`P_Deleted`（RV-26）・参照の展開（RV-31）まで実装済み。
「API でやれることをできるようにする」の残りは、機能ではなく上表の **D1〜D3** に集約されている。

読み方 — 未実装リソースは descriptor パターン（`src/resources/client.ts` は 89 行）に載るため 1 種あたりは小さく、
**7 種のうち 6 種はそのまま横展開できる**。**Phase だけは alias 接頭辞が無く**（`Id` / `Resource` / `ResourceId`）
汎用 factory の前提から外れる＝別途設計が要る。Contract / Sales の `Currency` は **Data Type が `Number`** なので
新しい型は不要（Field Type と Data Type は別物）。
