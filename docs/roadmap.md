...
| V2  | **使い方のドキュメント完成**           | 「入門（順に読む）→ 考え方 → 目的別 HOWTO」が揃い、**読者の目的から引ける**こと。検査は [ADR-0070][adr70] の 4 つ                          | 4 層すべてと検査①②④が済み、`docs/usage/` に集約（ADR-0071）／**残るは C（README を絞る ＋ 検査③）** |
...
- [ ] **使い方のドキュメント（V2）** — [ADR-0070][adr70]（accepted）に沿って進行中。3 本に分けた。
  ...
  - [x] ✅ **移設**（2026-09-12）— [ADR-0071][adr71] で利用者向け 4 層を `docs/usage/` へ集め、
        ...
        `SENTINELS` と同じ番人を足した
  - [ ] **C: README を絞る ＋ 検査③**（公開メソッドが HOWTO から辿れる）
