...
| V3  | **自分で進められる作業が残っていない** | 本書の「着手可能」「判断待ち」「要 ADR」がすべて 0 件 ／ [findings][findings] の open が 0 件                                              | **未達に戻った**（2026-09-21・着手可能 1＝ [ADR-0087][adr87] の実装）。判断待ち 0・要 ADR 0（条件付き 1 件＝ [RV-54][rv54] 案 (b)・実例が出たら起票。分母に数えない）・open 0。README 英語版は `1.0.0` の後へ |
...
- [ ] **カスタム項目の宣言を `tenant(id, { fields })` に移す**（[ADR-0087][adr87]・2026-09-21 accepted）—
      `PortersClientOptions` から `fields` を外し、`PortersClient` / `PortersClientOptions` を非ジェネリックに。
      **破壊的**（移行は 1 対 1・CHANGELOG 最上部に手順）。型テストで両方向を pin し、ガイド 2 本・概念 1 本・
      基本設計・examples・ADR-0005 / 0023 の注記まで洗う（ADR の「実施時の合意事項」1〜6）。
...
| ----------------------------- | -------------------------------------------------- | ------------------------------------------------------------------ |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 1・判断待ち 0・要 ADR 0（V3 は着手可能が 0 に戻れば達成） |
| [findings][findings]          | レビュー指摘の処置台帳（RV-N）                     | **open は 0 件** ✅                                                |
| [docs/adr][adr]               | 【accept 済み・実装済み】＋論点バックログ          | decide 待ち 0 件・反映待ち 1 件（[ADR-0087][adr87] の実装）        |
