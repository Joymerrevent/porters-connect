...
| V3  | **自分で進められる作業が残っていない** | 本書の「着手可能」「判断待ち」「要 ADR」がすべて 0 件 ／ [findings][findings] の open が 0 件                                              | 着手可能 3・判断待ち **1**・要 ADR 0・**open 0** ✅                                                                       |
...
- [x] ✅ 案D **`defineFields` 深掘りの残り**（[ADR-0023][adr23] D5）— **完了**（2026-09-10）。
      ...
      両方とも実装済み・未リリース。
  - [x] ✅ **テナント突合＋宣言生成** → [ADR-0069][adr69]・**実装済み・未リリース**（2026-09-10）。
...
- [ ] **使い方のドキュメント（V2）** — [ADR-0070][adr70]（accepted）に沿って進行中。3 本に分けた。
...
- [ ] **V1（機能網羅）の残り** — [エンドポイント × 機能マトリクス][coverage]は作成済みで、
      reference ↔ 表 ↔ 実装を両方向で突き合わせる検査（`test/integration/endpoint-coverage.test.ts`・94 検査）も入った。
      残っているのは**表 A と表 B/C がずれている 8 セルの決着**（起票時は 10 セル。Phase の 2 つは
      [ADR-0076][adr76] で解消した）。
  - **ADR で決着済み（2 セル）**: Phase の `resourceId` / `id` は `condition` で書く（[ADR-0061][adr61]）
  - **ライブ検証待ち（6 セル）**: すべて Attachment — `requestType` / `resource` / `resourceId` /
    `id` / `field` / `condition`（[LV-3][lv] / [LV-24][lv]）。**出典どおりなら公開サーフェスの
    改定になる**ので、確定したら ADR を起こす ＝ V1 の「非対応セルは ADR 番号必須」を満たすのは
    V5 の後になるセルがある
- [x] ✅ **Phase の Read クエリを出典に揃える**（[ADR-0076][adr76]）— **実装済み・未リリース**（2026-09-16）。
...
- [x] ✅ **Attachment の `searchAll` と本体の運び方**（[ADR-0075][adr75]）— **実装済み・未リリース**（2026-09-16）。
...
| ----------------------------- | -------------------------------------------------- | ---------------------------------------- |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 3・判断待ち 1・要 ADR 0（＝V3） |
...
- ADR 0001〜0069（0037 は 0039 で・**0033 は 0060 で**・0065 / 0066 は 0067 で superseded／
  ...
  **実装待ちは無し**（[ADR-0068][adr68] / [ADR-0069][adr69] とも**実装済み・未リリース**）。[索引][adr] が正
...
- 品質ゲート green・**970 tests**／project-review プロセス＋台帳（[findings][findings]：
