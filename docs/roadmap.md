...
| ID  | 条件                                   | 何で測るか                                                                                                                                 | 現在地                                                                                                |
| --- | -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------- |
| V1  | **機能網羅**（エンドポイント単位）     | 「**API エンドポイント × 機能**」のマトリクスを reference から起こし、実装と**両方向**で突合（D4 と同じ形）。**非対応セルは ADR 番号必須** | **マトリクス作成・検査あり**（[表 A〜G][coverage]）。残るずれ 10 セル ＝ ADR 決着 2・ライブ検証待ち 8 |
...
- [ ] **V1（機能網羅）の残り** — [エンドポイント × 機能マトリクス][coverage]は作成済みで、
      ...
      残っているのは**表 A と表 B/C がずれている 10 セルの決着**。
  - **ADR で決着済み（2 セル）**: Phase の `resourceId` / `id` は `condition` で書く（[ADR-0061][adr61]）
  - **ライブ検証待ち（8 セル）**: Attachment の `requestType` / `resource` / `resourceId` / `id` /
    `field` / `condition`（[LV-3][lv] / [LV-24][lv]）と、Phase の `keywords` / `itemstate`（**新規 [LV-25][lv]**）。
    **出典どおりなら公開サーフェスの改定になる**ので、確定したら ADR を起こす ＝ V1 の
    「非対応セルは ADR 番号必須」を満たすのは V5 の後になるセルがある
- [ ] **Attachment に `searchAll` を足すか**（[RV-45][rv45]・open）— マトリクスの表 D で見つかった。
      `start` は取れるのに `searchAll` だけ無く、**無い理由も記録されていない**唯一のセル。
      足す／足さないのどちらでもよいが、決めて記録する必要がある
