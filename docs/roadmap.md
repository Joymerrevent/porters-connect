...
**実装は着手可能**（PR 4 本を順に。下記）＝ V3 は実装が終わるまで未達に戻る。
...
| V2  | **使い方のドキュメント完成**           | 「入門（順に読む）→ 考え方 → 目的別 HOWTO」が揃い、**読者の目的から引ける**こと。検査は [ADR-0070][adr70] の 5 つ                          | **達成**（4 層 ＋ 検査①〜⑤ ＋ README を入口に絞る ＋ `docs/usage/` への集約）。**5 章への組み直しは着手可能**（[ADR-0088][adr88]・2026-09-22 accepted。達成の定義は 5 章に読み替える）                                                                                                                                                                                     |
| V3  | **自分で進められる作業が残っていない** | 本書の「着手可能」「判断待ち」「要 ADR」がすべて 0 件 ／ [findings][findings] の open が 0 件                                              | **着手可能 1**（2026-09-22・[ADR-0088][adr88] 使い方ドキュメントの 5 章化の実装・PR 4 本）。それ以外は達成のまま（2026-09-21・run 2 の open 4 件＝ [RV-58][rv58]〜61 は同日にすべて処置・open 0。[RV-59][rv59] 案 (b) は「0015 をそのまま適用」で決着＝要 ADR 0）。条件付き 1 件（[RV-54][rv54] 案 (b)・実例が出たら起票。分母に数えない）。README 英語版は `1.0.0` の後へ |
...
| ----------------------------- | -------------------------------------------------- | --------------------------------------------------------- |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 1・判断待ち 0・要 ADR 0                          |
| [findings][findings]          | レビュー指摘の処置台帳（RV-N）                     | **open は 0 件** ✅                                       |
| [docs/adr][adr]               | 【accept 済み・実装済み】＋論点バックログ          | **decide 待ち 0・反映待ち 1**（[ADR-0088][adr88] の実装） |
