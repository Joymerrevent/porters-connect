...
実行時にすべて弾く — [ADR-0092][adr92]（[ADR-0087][adr87] / [ADR-0091][adr91] の `never` の型付けを改める）。実装は**着手可能**。
...
- [ ] **構築オプションと `tenant()` のオプションで、定義していないキーを実行時にすべて弾く**（[ADR-0092][adr92]・2026-09-24 accepted・
...
| **判断待ち** | **0 件** ✅（[ADR-0092][adr92] は 2026-09-24 に accepted）                                               |
...
| ----------------------------- | -------------------------------------------------- | ------------------------------------------------------------------ |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 1（[ADR-0092][adr92] の実装）・判断待ち 0・要 ADR 0       |
| [findings][findings]          | レビュー指摘の処置台帳（RV-N）                     | **open は 0 件** ✅                                                |
| [docs/adr][adr]               | 【accept 済み・実装済み】＋論点バックログ          | **decide 待ち 0 件** ✅・反映待ち 1 件（[ADR-0092][adr92] の実装） |
