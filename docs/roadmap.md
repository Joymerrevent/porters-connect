...
反映（PRD §7・CI に `pnpm sandbox`・レビュー手順）が残っている。**V5・V6 は契約待ち**なので、
...
- [x] ✅ **成功指標の数値化タイミング**（PRD [§8][prd]）— [ADR-0084][adr84] で決着（2026-09-18・
      ...
      **ベースラインとみなせる実利用が 1 件も無かった**（npm の DL はミラー由来＝publish 日に山／
      ...
      反映（PRD §7・CI に `pnpm sandbox`・レビュー手順・§8 の解決済み化）は別 PR。
...
| [docs/adr][adr]               | 【accept 済み・実装済み】＋論点バックログ          | decide 待ち 0・**反映待ち 1 件**（[ADR-0084][adr84]） |
