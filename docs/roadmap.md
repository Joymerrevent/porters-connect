...
| V3  | **自分で進められる作業が残っていない** | 本書の「着手可能」「判断待ち」「要 ADR」がすべて 0 件 ／ [findings][findings] の open が 0 件                                              | **着手可能 1 件・open 1 件**（2026-09-24・[RV-63][rv63] を [ADR-0091][adr91] として起票し accepted・実装待ち。[ADR-0090][adr90] は accepted・同日実装。[RV-63][rv63] は条件付き＝実例待ち）。それまでは着手可能・判断待ち・要 ADR とも 0（2026-09-24・[RV-62][rv62] は [ADR-0089][adr89] を accepted にして同日実装・fixed）。それまでは達成（2026-09-22 に [ADR-0088][adr88] の実装が着手可能に 1 件戻ったが、同日 [#378][pr378] で完了）。それ以外も達成のまま（2026-09-21・run 2 の open 4 件＝ [RV-58][rv58]〜61 は同日にすべて処置・open 0。[RV-59][rv59] 案 (b) は「0015 をそのまま適用」で決着＝要 ADR 0）。条件付き 1 件（[RV-54][rv54] 案 (b)・実例が出たら起票。分母に数えない）。README 英語版は `1.0.0` の後へ |
...
- [ ] **トークンの取得と保存を別々に受け取り、管理は `PortersClient` が受け持つ**（[ADR-0091][adr91]・2026-09-24 accepted・
...
| ----------------------------- | -------------------------------------------------- | --------------------------------------------------------------- |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 1（[ADR-0091][adr91] の実装）・判断待ち 0・要 ADR 0    |
| [findings][findings]          | レビュー指摘の処置台帳（RV-N）                     | **open は 1 件**（[RV-63][rv63]・[ADR-0091][adr91] の実装待ち） |
