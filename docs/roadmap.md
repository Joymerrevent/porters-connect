...
| V2  | **リファレンス完成**                   | 公開 API（`src/index.ts` の export）が 1 つ残らずリファレンスに載っている                                                                  | ✅ **達成**（2026-09-10・177 記号すべてに頁）   |
| V3  | **自分で進められる作業が残っていない** | 本書の「着手可能」「判断待ち」「要 ADR」がすべて 0 件 ／ [findings][findings] の open が 0 件                                              | 着手可能 1・判断待ち 3・要 ADR 1・**open 0** ✅ |
...
- [x] ✅ **API リファレンスの実装**（[ADR-0068][adr68]）— **完了・未リリース**（2026-09-10）＝ **V2 達成**。
      TypeDoc + `typedoc-plugin-markdown` で `docs/api/`（176 ページ）に生成してコミットし、
      `pnpm check:api` が CI で再生成差分を検査する。着手時の 4 点は実測して確定（ADR に記録）—
      **ソースリンクは `gitRevision: "main"`**（SHA 埋め込みだと 175/176 ファイルが毎コミットで変わるが、
      ブランチ名なら定義位置が動いた 2 ファイルだけ）
