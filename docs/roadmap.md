...
**2026-09-24 に判断待ちが 1 件に戻った**: 構築オプションと `tenant()` のオプションで、定義していないキーを
実行時にすべて弾くか — [ADR-0092][adr92]（[ADR-0087][adr87] / [ADR-0091][adr91] の `never` の型付けを改める案）。
...
- [ ] **構築オプションと `tenant()` のオプションで、定義していないキーを実行時にすべて弾くか** — [ADR-0092][adr92]
      （2026-09-24 起票・**proposed**・stakeholder の指摘）。推奨: `PortersClientOptions` から `auth?: never` / `fields?: never` を外し、
      構築時と `tenant()` の呼び出し時に、定義していないキー（値が `undefined` のものを除く）を `PortersConfigError` で止める。
      廃止した名前を特別に案内しない。許可するキーの一覧は型と `satisfies` で突き合わせる（破壊的）。
...
| **判断待ち** | **1 件**（[ADR-0092][adr92]・2026-09-24 起票）                                                           |
...
| ----------------------------- | -------------------------------------------------- | -------------------------------------------------------- |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 0・判断待ち 1（[ADR-0092][adr92]）・要 ADR 0    |
| [findings][findings]          | レビュー指摘の処置台帳（RV-N）                     | **open は 0 件** ✅                                      |
| [docs/adr][adr]               | 【accept 済み・実装済み】＋論点バックログ          | **decide 待ち 1 件**（[ADR-0092][adr92]）・反映待ち 0 件 |
