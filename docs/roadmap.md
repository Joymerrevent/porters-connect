...
**2026-09-25 に判断待ちが 2 件になった**: 読み取りの戻り値の型を `field` で絞るか — [ADR-0096][adr96]
（[ADR-0095][adr95] の議論から分けた。推奨は読んでいない項目のキーを型から外し、読んだ項目は省略可能のままにする案）。

**2026-09-25 に判断待ちが 1 件に戻った**: 複数の ID でまとめて読む `getMany(ids)` を足すか — [ADR-0095][adr95]
（stakeholder の問い。PORTERS の出典は `P_Id` の `or` について記述が割れているので、返ってきた ID を突き合わせて
前提が外れたらエラーで止める案）。
...
| **判断待ち** | **2 件**（[ADR-0095][adr95]・[ADR-0096][adr96]・2026-09-25 起票）                                        |
...
| ----------------------------- | -------------------------------------------------- | --------------------------------------------------------------------------- |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 0・判断待ち 2（[ADR-0095][adr95]・[ADR-0096][adr96]）・要 ADR 0    |
| [findings][findings]          | レビュー指摘の処置台帳（RV-N）                     | **open は 0 件** ✅                                                         |
| [docs/adr][adr]               | 【accept 済み・実装済み】＋論点バックログ          | **decide 待ち 2 件**（[ADR-0095][adr95]・[ADR-0096][adr96]）・反映待ち 0 件 |
