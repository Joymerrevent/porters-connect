...
| V2  | **使い方のドキュメント完成**           | 「入門（順に読む）→ 考え方 → 目的別 HOWTO」が揃い、**読者の目的から引ける**こと。検査は [ADR-0070][adr70] の 4 つ                          | 入門 5 本・考え方 5 本・検査①②④は済／**残るは目的別 3 本と C（README ＋ 検査③）** |
...
- [ ] **使い方のドキュメント（V2）** — [ADR-0070][adr70]（accepted）に沿って進行中。3 本に分けた。
  ...
  - [ ] **D: 目的別の残り 3 本**（添付ファイル／毎日同期するバッチ／契約なしでテストを書く）。
        ADR-0070 の構成では howto/ に 8 本あり、6 本が既存の移設で埋まっている
