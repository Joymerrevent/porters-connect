...
- **[LV-16][lv] に `VERIFY(live)` を設置済み**（いまは `src/accessor/expand.ts` の `expandEntry`）。
  外れても直すのは要求文字列だけ。
...
| ----------------------------- | -------------------------------------------------- | ---------------------------------------------------------------------- |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 1（[ADR-0101][adr101]）・判断待ち 0・要 ADR 0                 |
| [findings][findings]          | レビュー指摘の処置台帳（RV-N）                     | **open は 0 件** ✅                                                    |
| [docs/adr][adr]               | 【accept 済み・実装済み】＋論点バックログ          | decide 待ち 0 件・**反映待ち 1 件**（[ADR-0101][adr101] の追記の実装） |
