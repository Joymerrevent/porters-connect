...
**最新は 0.19.0**（2026-09-18）＝ CJS からの入口を `require` 条件で開け、Node の下限を 22.12 に
...
**判断待ちは 0 件になった**（2026-09-18・[ADR-0084][adr84] が最後の 1 件）。**V1・V2 は達成**し、
findings の open も 0。V3 に残るのは **任意の README 英語版**だけで、あとは [ADR-0084][adr84] の
反映も済んだ（PRD §7・CI に `pnpm sandbox`・レビュー手順）。**V5・V6 は契約待ち**なので、
自分で進められる作業はほぼ尽きている＝ここから先は「契約を取る」が最大のボトルネック。
...
| ID  | 条件                                   | 何で測るか                                                                                                                                 | 現在地                                                                                                         |
| --- | -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------- |
| V1  | **機能網羅**（エンドポイント単位）     | 「**API エンドポイント × 機能**」のマトリクスを reference から起こし、実装と**両方向**で突合（D4 と同じ形）。**非対応セルは ADR 番号必須** | **達成**（[表 A〜G][coverage]・検査 188 件）。**根拠の無いずれは 0** ＝ 意図的な非対応 3 セルはすべて ADR 付き |
| V2  | **使い方のドキュメント完成**           | 「入門（順に読む）→ 考え方 → 目的別 HOWTO」が揃い、**読者の目的から引ける**こと。検査は [ADR-0070][adr70] の 5 つ                          | **達成**（4 層 ＋ 検査①〜⑤ ＋ README を入口に絞る ＋ `docs/usage/` への集約）                                  |
| V3  | **自分で進められる作業が残っていない** | 本書の「着手可能」「判断待ち」「要 ADR」がすべて 0 件 ／ [findings][findings] の open が 0 件                                              | 着手可能 **1**（任意の README 英語版のみ）・**判断待ち 0** ✅・要 ADR 0・**open 0**                            |
...
- 案D `defineFields` の残りと README 英語版は、**「着手可能」へ移した**（第1層の完成が向き先になったため）。
...
| ----------------------------- | -------------------------------------------------- | ---------------------------------------- |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 1・判断待ち 0・要 ADR 0（＝V3） |
...
- [x] 公開済み — **`@joymerrevent/porters-connect@0.19.0`**（npm latest・2026-09-18 にレジストリで確認・**8 files / 872.7 kB**）。**全 25 版**を半自動フローでリリース:
      0.1.0 → 0.1.1 → 0.2.0 → 0.2.1 → 0.3.0 → 0.4.0 → 0.5.0 → 0.6.0 → 0.6.1 → 0.6.2 → 0.7.0 → 0.8.0 → 0.9.0 → 0.10.0 → 0.11.0 → 0.12.0 → 0.12.1 → 0.13.0 → 0.14.0 → 0.15.0 → 0.15.1 → 0.16.0 → 0.17.0 → 0.18.0 → 0.19.0
...
- [x] publish ワークフロー `release.yml`（**Release 公開**で起動・**OIDC Trusted Publishing**・**NPM_TOKEN 不要**・provenance 自動）＋ npm 側の信頼登録済み（0.1.0〜0.19.0 の**全 25 版**で運用実績あり）
