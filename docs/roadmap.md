...
| V3  | **自分で進められる作業が残っていない** | 本書の「着手可能」「判断待ち」「要 ADR」がすべて 0 件 ／ [findings][findings] の open が 0 件                                              | **未達に戻った**（2026-09-21 run 2 で open 4 件＝ [RV-58][rv58]〜61。RV-58 / 59 は同日処置）。**要 ADR 1**（判断待ち＝ [RV-59][rv59] 案 (b)・survivor 79 件を撃破するか 0015 を改訂するか）・条件付き 1 件（[RV-54][rv54] 案 (b)・実例が出たら起票。分母に数えない）。README 英語版は `1.0.0` の後へ |
...
- **判断待ち 1 件（着手可能）**: Stryker の survivor 79 件を**テストで撃破する**（[ADR-0015][adr15] の規則
  どおり・ADR 不要）か、「エラー文言の文字列変異は pin しない」を **0015 に足す**（規則の改訂＝要 ADR）か
  — [RV-59][rv59] 案 (b)。RV-59 の処置は `client.ts` の 10 件（同値でなかった）を撃破しただけで、
  残りの扱いは決めていない（2026-09-21）。置き場は [ADR README の論点バックログ][adr-backlog]【プロセス】。
  件数でなく**行の一覧**で判断する（前 run が件数だけ見て「同値」と記録した経緯）。
