...
**2026-09-25 に判断待ちが 1 件に戻った**: PORTERS が決めた値と定義表を `src/porters/` にまとめるか — [ADR-0098][adr98]
（stakeholder の提案。[ADR-0097][adr97] の実装を見て「`resources/` 直下はリソースだけにしたい」「PORTERS 側の決まりを責務で
1 か所にまとめたい」。案B＝定義表と上限・値・対応表をまとめ、`porters/` を一番下の層にする）。
...
- [ ] **PORTERS が決めた値と定義表を `src/porters/` にまとめるか** — [ADR-0098][adr98]（2026-09-25 起票・**proposed**・stakeholder の提案）。
      案B: resource-list / field-type と、出典に書いてある上限・値・対応表（`DataType` の一覧を含む）を `src/porters/` に移し、
      何も import しない一番下の層にする。ライブラリの判断（分類・安全率・既定値）と、値を使うコードは今の場所に残す。
      `resource-list` は向きを逆にし、各リソースの定義を表の名前の型で型付けする。公開 API は変えない。
...
| **判断待ち** | **1 件**（[ADR-0098][adr98]・2026-09-25 起票）                                                           |
...
| ----------------------------- | -------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 1（[ADR-0096][adr96]）・判断待ち 1（[ADR-0098][adr98]）・要 ADR 0               |
| [findings][findings]          | レビュー指摘の処置台帳（RV-N）                     | **open は 0 件** ✅                                                                      |
| [docs/adr][adr]               | 【accept 済み・実装済み】＋論点バックログ          | **decide 待ち 1 件**（[ADR-0098][adr98]）・**反映待ち 1 件**（[ADR-0096][adr96] の実装） |
