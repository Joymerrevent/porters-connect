...
| V2  | **リファレンス完成**                   | 公開 API（`src/index.ts` の export）が 1 つ残らずリファレンスに載っている                                                                  | 方式が [ADR-0068][adr68] で議論中 |
| V3  | **自分で進められる作業が残っていない** | 本書の「着手可能」「判断待ち」「要 ADR」がすべて 0 件 ／ [findings][findings] の open が 0 件                                              | 着手可能 2・判断待ち 4・要 ADR 1  |
...
- [ ] **API リファレンスの作り方**（[ADR-0068][adr68]・`proposed`）— **V2 の前提**。TypeDoc 生成 /
      手書き / 静的サイトのどれを採るか。推奨は「TypeDoc → Markdown を `docs/api/` に生成してコミット
      ＋ CI で再生成差分を検査」（生成漏れが CI で落ちる＝人の注意に頼らない）
...
| **判断待ち** | 上記「判断待ち」節を参照（CJS 出力・Sales の必須・成功指標の数値化・[ADR-0068][adr68]）＝ **V3**    |
...
| ----------------------------- | -------------------------------------------------- | ---------------------------------------- |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 2・判断待ち 4・要 ADR 1（＝V3） |
