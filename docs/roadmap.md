...
**2026-09-24 に判断待ちが 1 件に戻った**: `porters.auth.getToken()` が Access Token の期限も返すようにするか —
[ADR-0093][adr93]（[ADR-0034][adr34] SD-6 の `Promise<string>` を改める案）。
...
- [ ] **`porters.auth.getToken()` が Access Token の期限も返すようにするか** — [ADR-0093][adr93]（2026-09-24 起票・**proposed**・
      stakeholder との議論）。推奨: 戻り値を `IssuedToken`（`{ token, expiresAt? }`）に変える（破壊的）。Refresh Token は返さない。
      中央のサービスがアプリへ期限つきでトークンを渡す実践例の前提。
...
| **判断待ち** | **1 件**（[ADR-0093][adr93]・2026-09-24 起票）                                                           |
...
| ----------------------------- | -------------------------------------------------- | -------------------------------------------------------- |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 0・判断待ち 1（[ADR-0093][adr93]）・要 ADR 0    |
| [findings][findings]          | レビュー指摘の処置台帳（RV-N）                     | **open は 0 件** ✅                                      |
| [docs/adr][adr]               | 【accept 済み・実装済み】＋論点バックログ          | **decide 待ち 1 件**（[ADR-0093][adr93]）・反映待ち 0 件 |
