...
[ADR-0093][adr93]（案A＝戻り値を `IssuedToken` に。[ADR-0034][adr34] SD-6 を改める）。実装は**着手可能**。
...
- [ ] **`porters.auth.getToken()` が Access Token の期限も返す**（[ADR-0093][adr93]・2026-09-24 accepted・stakeholder との議論）。
      決定: 戻り値を `IssuedToken`（`{ token, expiresAt? }`）に変える（破壊的）。Refresh Token は返さない。
      あとに続く作業: 実践例「トークンを DB に保存する（`tokenStore`）」と「中央のサービスから `tokenProvider` で受け取る」。
...
| ----------------------------- | -------------------------------------------------- | ------------------------------------------------------------------ |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 1（[ADR-0093][adr93] の実装）・判断待ち 0・要 ADR 0       |
| [findings][findings]          | レビュー指摘の処置台帳（RV-N）                     | **open は 0 件** ✅                                                |
| [docs/adr][adr]               | 【accept 済み・実装済み】＋論点バックログ          | **decide 待ち 0 件** ✅・反映待ち 1 件（[ADR-0093][adr93] の実装） |
