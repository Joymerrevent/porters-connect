...
**2026-09-25 に判断待ちが 1 件に戻った**: PR のミューテーションテストを変更したファイルだけにし、`main` への PR は必ず
フル run にするか — [ADR-0094][adr94]（[ADR-0015][adr15] の「PR もフル run」と [ADR-0028][adr28] の「リリースの流れの PR は
develop と同じなら skip」を改める案）。
...
- [ ] **PR のミューテーションテストを変更したファイルだけにし、`main` への PR は必ずフル run にするか** — [ADR-0094][adr94]
      （2026-09-25 起票・**proposed**・stakeholder の指摘）。推奨: 機能 PR は変更した `src` のファイル（と隣のテストが変わった実装）だけを
...
| **判断待ち** | **1 件**（[ADR-0094][adr94]・2026-09-25 起票）                                                           |
...
| ----------------------------- | -------------------------------------------------- | -------------------------------------------------------- |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 0・判断待ち 1（[ADR-0094][adr94]）・要 ADR 0    |
| [findings][findings]          | レビュー指摘の処置台帳（RV-N）                     | **open は 0 件** ✅                                      |
| [docs/adr][adr]               | 【accept 済み・実装済み】＋論点バックログ          | **decide 待ち 1 件**（[ADR-0094][adr94]）・反映待ち 0 件 |
