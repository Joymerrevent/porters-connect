...
**主軸「全リソース網羅 ＋ ドキュメント充実」（[ADR-0060][adr60]）は D1 / D2 / D4 / D5 が完了**。
D1 は 0.12.0 で公開済み（2026-08-31・データ系 13/13）、**D2（マスタ項目）は 2026-09-04 に完了**
（User 4→17・突合検査をマスタへ拡張）。**残るのは D3（`Link` / `Image`）だけで、これは要 ADR**。
品質ゲートは全 green（**811 tests**・coverage は perFile 100%）。

### 着手可能（ブロック無し）

- [ ] **D3: `Link` / `Image` の実装** — [ADR-0064][adr64] が **accepted**（2026-09-05・推奨案を全採用）。
      Image は既定 `FileName` のみ ＋ 専用 `image` オプションで `Content` を選択、Write は
      サイズガードを外すかわりに 2MB / 255 バイト / mime 4 種を送信前に検査（一括書き込みは弾く）。
      Link は形で判別する union。builder に `f.image()` / `f.link()` を足す。**これで D3 が 17/17**
...
| 待ちの種類   | 中身                                                                                                |
| ------------ | --------------------------------------------------------------------------------------------------- |
| **契約待ち** | ライブ検証 **LV-1〜18**（[live-verification][lv]）。リリースのブロッカーではない                    |
...
| ----------------------------- | -------------------------------------------------- | --------------------------------------- |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 1 件（D3 の実装）              |
| [findings][findings]          | レビュー指摘の処置台帳（RV-N）                     | **open は 0 件**                        |
| [docs/adr][adr]               | 【accept 済み・実装済み】＋論点バックログ          | **0064 は accepted・実装待ち**          |
...
「API でやれることをできるようにする」の残りは、機能ではなく上表の **D3** だけになった。
...
- 品質ゲート green・**811 tests**／project-review プロセス＋台帳（[findings][findings]：**open は 0 件**。
...
実 PORTERS 契約が無いと確定できない仮定は [live-verification][lv]（**LV-1〜18**）に集約。リリースのブロッカーではないが、
