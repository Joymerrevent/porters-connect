...
> ⚠️ **「完成」の定義を更新する必要がある**。本書が [ADR-0060][adr60] の根拠として引いている
> stakeholder の定義は「**ライブラリの完成＝全リソースにアクセスでき、API でやれることをできるように
> する ＋ 使い方のドキュメント充実**」（下記「なぜ順序を変えたか」）で、**これは D1〜D5 の完了で
> 満たされた**。つまり旧定義では既に「完成」に達している。
>
> 一方で現在の版は `0.14.0` で、**`1.0.0` を何をもって出すかはどこにも書かれていない**（PRD にも
> 本書にも無い）。下の「着手可能」を全部やっても、それが完成かどうかを判定する基準が無い。
> **完成条件の再定義が最初の 1 手**になる（判断待ちの先頭に置いた）。
...
- [ ] ⭐ **`1.0.0` の完成条件を決める** — 旧定義（全リソース ＋ ドキュメント）は D1〜D5 で満たしたので、
      **次の終点を決め直す**。候補は「公開 API の凍結宣言」「CJS 出力」「LV の確定（契約が要る）」
      「英語ドキュメント」など。**これが決まらないと『完成を進める』の終点が無い**。
      決めたら本書に完了条件（D1〜D5 と同じ形＝検査できる形）で書き下す
- [ ] **v1 で CJS 出力まで出すか**（PRD [§8][prd]・[eng]）— 完成条件に直結するので上と一緒に決めたい。
      CLAUDE.md は「ESM 前提・可能なら CJS も出力」と書いており、現状は **ESM のみ**
...
| 待ちの種類   | 中身                                                                                                |
| ------------ | --------------------------------------------------------------------------------------------------- |
| **契約待ち** | ライブ検証 **LV-1〜22**（[live-verification][lv]）。リリースのブロッカーではない                    |
| **需要待ち** | フェイクサーバー **フェーズ7**（package 昇格・配布。[実装計画][fake-plan]・stakeholder 2026-08-09） |
| **判断待ち** | 上記「判断待ち」節を参照（**`1.0.0` の完成条件**・CJS 出力・Sales の必須・成功指標の数値化）        |
...
| ----------------------------- | -------------------------------------------------- | --------------------------------------- |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 **2 件**（完成条件は判断待ち） |
