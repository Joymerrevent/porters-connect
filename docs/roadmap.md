...
**2026-09-25 に判断待ちがもう 1 件増えた**: 読み取りの戻り値の型を `field` で絞るか — [ADR-0096][adr96]
（[ADR-0095][adr95] の議論から分けた。推奨は読んでいない項目のキーを型から外し、読んだ項目は省略可能のままにする案）。
...
| V3  | **自分で進められる作業が残っていない** | 本書の「着手可能」「判断待ち」「要 ADR」がすべて 0 件 ／ [findings][findings] の open が 0 件                                              | **着手可能 1 件・判断待ち 1 件**（2026-09-25・[ADR-0095][adr95] を起票し accepted・実装待ち。[ADR-0096][adr96] は proposed）。それまでは達成に戻っていた（2026-09-25・[ADR-0094][adr94] を起票・accepted・同日実装。着手可能・判断待ち・要 ADR とも 0）。それまでは達成（2026-09-24・[ADR-0093][adr93] を起票・accepted・同日実装し、続く実践例 2 本も同日に足した。着手可能・判断待ち・要 ADR とも 0）。それまでは達成（2026-09-24・[ADR-0092][adr92] を起票・accepted・同日実装。着手可能・判断待ち・要 ADR とも 0）。それまでは達成（2026-09-24・[RV-63][rv63] を [ADR-0091][adr91] として起票・accepted・同日実装・fixed。[ADR-0090][adr90] も同日実装。着手可能・判断待ち・要 ADR・open とも 0）。それまでは着手可能・判断待ち・要 ADR とも 0（2026-09-24・[RV-62][rv62] は [ADR-0089][adr89] を accepted にして同日実装・fixed）。それまでは達成（2026-09-22 に [ADR-0088][adr88] の実装が着手可能に 1 件戻ったが、同日 [#378][pr378] で完了）。それ以外も達成のまま（2026-09-21・run 2 の open 4 件＝ [RV-58][rv58]〜61 は同日にすべて処置・open 0。[RV-59][rv59] 案 (b) は「0015 をそのまま適用」で決着＝要 ADR 0）。条件付き 1 件（[RV-54][rv54] 案 (b)・実例が出たら起票。分母に数えない）。README 英語版は `1.0.0` の後へ |
| V4  | **残る未確認はライブ検証だけ**         | **`pnpm check:lv` が緑**＝コードの `VERIFY(live)` と [live-verification][lv] のエントリが双方向で対応する（[RV-50][rv50]）                 | **達成**（検査が CI に載った。未確認 29 ／ 確定 1 ／ 解消 2）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| V5  | **契約後、その検証を全件確定させる**   | [live-verification][lv] の **`未確認` が 0 件**（`確定` と `解消` はどちらも終端 — [状態の意味][lvstate]）                                 | **未確認 29 件**（契約待ち）。`確定` 1・`解消` 2                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
- [ ] **複数の ID でまとめて読む `getMany(ids)` を足す**（[ADR-0095][adr95]・2026-09-25 accepted・stakeholder の問い）。
      ...
      エラーで止める。戻り値は渡した順の `(T | undefined)[]`。Attachment とマスタには付けない。live-verification に `P_Id:or` の項目を足す。
...
- [ ] **読み取りの戻り値の型を `field` で絞るか** — [ADR-0096][adr96]（2026-09-25 起票・**proposed**・[ADR-0095][adr95] から分けた）。
      推奨: `search` / `searchAll` / `get` / `getMany` のすべてで、読んでいない項目のキーを型から外す（読んだ項目は省略可能のまま）。
      型の破壊的変更。実装は [ADR-0095][adr95] の実装の後。
...
| 待ちの種類   | 中身                                                                                                     |
| ------------ | -------------------------------------------------------------------------------------------------------- |
| **契約待ち** | ライブ検証の **`未確認` 29 件**（[live-verification][lv]）。0.x のブロッカーではないが **`1.0.0` の V5** |
| **需要待ち** | フェイクサーバー **フェーズ7**（package 昇格・配布。[実装計画][fake-plan]・stakeholder 2026-08-09）      |
| **判断待ち** | **1 件**（[ADR-0096][adr96]・2026-09-25 起票）                                                           |
...
| ----------------------------- | -------------------------------------------------- | ------------------------------------------------------------------------------------ |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 1（[ADR-0095][adr95]）・判断待ち 1（[ADR-0096][adr96]）・要 ADR 0           |
| [findings][findings]          | レビュー指摘の処置台帳（RV-N）                     | **open は 0 件** ✅                                                                  |
| [docs/adr][adr]               | 【accept 済み・実装済み】＋論点バックログ          | **decide 待ち 1 件**（[ADR-0096][adr96]）・反映待ち 1 件（[ADR-0095][adr95] の実装） |
| [live-verification][lv]       | 契約取得後に実機確認する仮定（LV-N）               | **未確認 29 件**（契約待ち）                                                         |
...
実 PORTERS 契約が無いと確定できない仮定は [live-verification][lv]（**LV-1〜32**・うち未確認 29）に集約。リリースのブロッカーではないが、
