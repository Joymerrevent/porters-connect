...
**2026-09-21 に判断待ちが 1 件に戻った**: カスタム項目の宣言（`fields`）を client でなく `tenant(id)` で
束ねるか — [ADR-0087][adr87] を起票（proposed・案B＝client から `fields` を外す、を推奨）。決まれば実装は別 PR。
...
| V3  | **自分で進められる作業が残っていない** | 本書の「着手可能」「判断待ち」「要 ADR」がすべて 0 件 ／ [findings][findings] の open が 0 件                                              | **未達に戻った**（2026-09-21・判断待ち 1＝ [ADR-0087][adr87]）。着手可能 0・要 ADR 0（条件付き 1 件＝ [RV-54][rv54] 案 (b)・実例が出たら起票。分母に数えない）・open 0。README 英語版は `1.0.0` の後へ |
...
- [ ] **カスタム項目の宣言（`fields`）を client でなく `tenant(id)` で束ねるか** — [ADR-0087][adr87]
      （2026-09-21 起票・proposed）。推奨は**案B＝client から `fields` を外し `tenant(id, { fields })` のみにする**
      （破壊的・移行は 1 対 1・`PortersClient` が非ジェネリックになる）。`A_` 共通 ＋ `U_` 固有は DSL の
      spread で合成できるので、ライブラリ側のマージは持たない。決まれば実装は別 PR。
...
| **判断待ち** | **1 件**（[ADR-0087][adr87]・カスタム項目の宣言の束ね先・2026-09-21 起票）                               |
...
| ----------------------------- | -------------------------------------------------- | ------------------------------------------------------------------ |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 0・判断待ち 1・要 ADR 0（V3 は判断待ちが 0 に戻れば達成） |
| [findings][findings]          | レビュー指摘の処置台帳（RV-N）                     | **open は 0 件** ✅                                                |
| [docs/adr][adr]               | 【accept 済み・実装済み】＋論点バックログ          | decide 待ち 1 件（[ADR-0087][adr87]）・反映待ち 0 件               |
