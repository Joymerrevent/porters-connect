...
- **Department - Read API**（2025/03・Connect API 8.2.1 で追加。Read のみ・スコープ `user_r`）が
  ...
  お知らせ記事と専用ページだけにある＝ 記事一覧の差分を取らないと見えない。→ 着手可能（下記）
- **Data Type: DateTime の時分型**（2026/08/04・PORTERS 9.3.0）。Field Type は年月日時分型と同じ 12 で
  **Field Read から区別できない**。基準日 `1970/01/01` 付きでしか書けない。→ 要 ADR（下記）
...
| ID  | 条件                                   | 何で測るか                                                                                                                                 | 現在地                                                                                                                                   |
| --- | -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------- |
| V1  | **機能網羅**（エンドポイント単位）     | 「**API エンドポイント × 機能**」のマトリクスを reference から起こし、実装と**両方向**で突合（D4 と同じ形）。**非対応セルは ADR 番号必須** | **1 本欠け**（2026-09-20）— `/v1/department` が表に無い（未実装・[表の注記][coverage]）。それ以外は達成（検査 188 件・根拠の無いずれ 0） |
| V2  | **使い方のドキュメント完成**           | 「入門（順に読む）→ 考え方 → 目的別 HOWTO」が揃い、**読者の目的から引ける**こと。検査は [ADR-0070][adr70] の 5 つ                          | **達成**（4 層 ＋ 検査①〜⑤ ＋ README を入口に絞る ＋ `docs/usage/` への集約）                                                            |
| V3  | **自分で進められる作業が残っていない** | 本書の「着手可能」「判断待ち」「要 ADR」がすべて 0 件 ／ [findings][findings] の open が 0 件                                              | 着手可能 **1**（Department）・判断待ち 0・要 ADR **1**（時分型）・open 0（2026-09-20 に戻った。README 英語版は `1.0.0` の後へ）          |
| V4  | **残る未確認はライブ検証だけ**         | **`pnpm check:lv` が緑**＝コードの `VERIFY(live)` と [live-verification][lv] のエントリが双方向で対応する（[RV-50][rv50]）                 | **達成**（検査が CI に載った。未確認 26 ／ 確定 1 ／ 解消 2）                                                                            |
| V5  | **契約後、その検証を全件確定させる**   | [live-verification][lv] の **`未確認` が 0 件**（`確定` と `解消` はどちらも終端 — [状態の意味][lvstate]）                                 | **未確認 26 件**（契約待ち）。`確定` 1・`解消` 2                                                                                         |
...
- [x] ✅ **V1（機能網羅）** — **達成**（2026-09-17）。[エンドポイント × 機能マトリクス][coverage]を
      ...
      （`test/integration/endpoint-coverage.test.ts`・**188 検査**）が CI で回っている。
...
- ~~（任意）**README 英語版**~~ → **`1.0.0` を出してから最後にやる**（2026-09-20・stakeholder）。
  ...
  下記「`1.0.0` の後」節
- [ ] **Department Read の実装**（マスタ 5 種目・2026-09-20 起票）— [reference][ref-department] は反映済み。
      `GET /v1/department?partition&count&start&field`・スコープは **`user_r`**（専用スコープ無し）・
      項目 6 つ（`P_Id` / `P_Name` / `P_Hidden` / `P_SortNo` / `P_RegistrationDate` / `P_UpdateDate`）・
      カスタム項目無し・Write 無し。Partition / User と同じマスタ factory（[ADR-0021][adr21] / [ADR-0022][adr22]）
      に載る見込みで、新しい抽象は要らないはず。**やること**: descriptor ＋ `t.department`・
      reference ↔ カタログ突合（`reference-catalog.test.ts`）・[マトリクス][coverage] 表 C / D に行を足して
      `endpoint-coverage.test.ts` を通す・フェイクサーバーに `/v1/department`・ガイド（リソース表）・
      `resourceNameOf` 等の一覧に入るか確認。応答の入れ子形は出典サンプルどおり（`<Department Total=…>`）。
...
- [ ] **DateTime の時分型（FT-12 の第 2 モード）をどう受けるか**（2026-09-20 起票）。出典は
      [field-data-types][ref-fdt] の「時分型」節。**論点**: ① Field Read から区別できないので、
      宣言（`defineFields` の builder・例 `f.time()`）で利用者に明示させるしかない ② 値の型 —
      ISO の `1970-01-01T09:00:00Z` で通すか、`"09:00"` / `"26:00"` のような時刻文字列に正規化するか
      （24:00〜47:59 が `1970/01/02` に化ける規則を隠すかどうか）③ 未宣言のまま FT-12 として扱った
      ときの倒れ方（現状: Read は値が欠けず届く／Write は `Z` 付きなら通り、オフセット付きは UTC 換算で
      ずれる＝黙ってずれるのがフェイルセーフに反する）④ condition の書式検証（基準日以外は Code 100）。
      **0.x のブロッカーではない**（時分型項目を使わない環境には影響が無い）が、管理者が項目を足した瞬間に
      既存連携が Code 103 で落ちる型の変更なので、ガイドに注意書きは先に入れてある。
...
| 待ちの種類   | 中身                                                                                                     |
| ------------ | -------------------------------------------------------------------------------------------------------- |
| **契約待ち** | ライブ検証の **`未確認` 26 件**（[live-verification][lv]）。0.x のブロッカーではないが **`1.0.0` の V5** |
| **需要待ち** | フェイクサーバー **フェーズ7**（package 昇格・配布。[実装計画][fake-plan]・stakeholder 2026-08-09）      |
| **判断待ち** | **0 件** ✅（成功指標は [ADR-0084][adr84]、Sales の必須は [ADR-0083][adr83] で決着）                     |
...
| ----------------------------- | -------------------------------------------------- | ------------------------------------------------------------------- |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 1・判断待ち 0・要 ADR 1（2026-09-20 に出典再取得で戻った） |
| [findings][findings]          | レビュー指摘の処置台帳（RV-N）                     | **open は 0 件** ✅                                                 |
| [docs/adr][adr]               | 【accept 済み・実装済み】＋論点バックログ          | **decide 待ち・反映待ちとも 0 件** ✅                               |
| [live-verification][lv]       | 契約取得後に実機確認する仮定（LV-N）               | **未確認 26 件**（契約待ち）                                        |
...
実 PORTERS 契約が無いと確定できない仮定は [live-verification][lv]（**LV-1〜29**・うち未確認 26）に集約。リリースのブロッカーではないが、
...
[adr21]: adr/0021-master-read-resources.md
