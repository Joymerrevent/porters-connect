...
Department は同日に実装して **V1 は再び達成**、時分型は同日に [ADR-0086][adr86] を起票 → accepted（実装が着手可能 1）:
...
| V3  | **自分で進められる作業が残っていない** | 本書の「着手可能」「判断待ち」「要 ADR」がすべて 0 件 ／ [findings][findings] の open が 0 件                                              | **着手可能 1**（[ADR-0086][adr86] の実装）・判断待ち 0・要 ADR 0・open 0（README 英語版は `1.0.0` の後へ）                                                                 |
| V4  | **残る未確認はライブ検証だけ**         | **`pnpm check:lv` が緑**＝コードの `VERIFY(live)` と [live-verification][lv] のエントリが双方向で対応する（[RV-50][rv50]）                 | **達成**（検査が CI に載った。未確認 27 ／ 確定 1 ／ 解消 2）                                                                                                              |
| V5  | **契約後、その検証を全件確定させる**   | [live-verification][lv] の **`未確認` が 0 件**（`確定` と `解消` はどちらも終端 — [状態の意味][lvstate]）                                 | **未確認 27 件**（契約待ち）。`確定` 1・`解消` 2                                                                                                                           |
...
- [ ] **時分型の変換関数の実装**（[ADR-0086][adr86]・2026-09-20 accepted）— `src/util/time-of-day.ts` に
      `decodeTimeOfDay`（ISO → `"HH:mm"`・基準日 1970/01/01・02 以外は `PortersConfigError`）と
      `encodeTimeOfDay`（`"HH:mm[:ss]"` 00:00〜47:59 → ISO・範囲外は `PortersConfigError`）を足して
      `src/index.ts` から公開。`generateFieldDecls` が FT-12 の行に注記を出す。ガイド（日時の概念・
      カスタム項目・ADR-0069 の突合が時分型を区別しないこと）と [field-data-types][ref-fdt] の
      「このライブラリでの扱い」を書き換え。秒が 00 以外で返る実例は LV を起票。changeset は minor。
      **型・カタログ・decode / encode・フェイクは触らない**（純関数 2 つ＋テスト＋ドキュメント）。
...
| 待ちの種類   | 中身                                                                                                     |
| ------------ | -------------------------------------------------------------------------------------------------------- |
| **契約待ち** | ライブ検証の **`未確認` 27 件**（[live-verification][lv]）。0.x のブロッカーではないが **`1.0.0` の V5** |
| **需要待ち** | フェイクサーバー **フェーズ7**（package 昇格・配布。[実装計画][fake-plan]・stakeholder 2026-08-09）      |
| **判断待ち** | **0 件** ✅（時分型は [ADR-0086][adr86] で決着・2026-09-20）                                             |
...
| ----------------------------- | -------------------------------------------------- | --------------------------------------------------- |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 1（ADR-0086 の実装）・判断待ち 0・要 ADR 0 |
| [findings][findings]          | レビュー指摘の処置台帳（RV-N）                     | **open は 0 件** ✅                                 |
| [docs/adr][adr]               | 【accept 済み・実装済み】＋論点バックログ          | decide 待ち 0・**反映待ち 1**（[ADR-0086][adr86]）  |
| [live-verification][lv]       | 契約取得後に実機確認する仮定（LV-N）               | **未確認 27 件**（契約待ち）                        |
...
実 PORTERS 契約が無いと確定できない仮定は [live-verification][lv]（**LV-1〜30**・うち未確認 27）に集約。リリースのブロッカーではないが、
