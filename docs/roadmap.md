...
| V3  | **自分で進められる作業が残っていない** | 本書の「着手可能」「判断待ち」「要 ADR」がすべて 0 件 ／ [findings][findings] の open が 0 件                                              | 着手可能 3・判断待ち 3・要 ADR 2・**open 1**（[RV-45][rv45]）                                                             |
...
- [ ] **Attachment の `searchAll` と本体の運び方**（[ADR-0075][adr75]・accepted）— [RV-45][rv45] の処置。
      `searchAll` を足し、**本体は `get(id)` でだけ運ぶ**（`search` からも外す＝**破壊的**）。
      実測の根拠は ADR にある（1 ファイル 2MB 超 × 200 件は V8 の文字列上限で読めない 等）
...
| ----------------------------- | -------------------------------------------------- | ---------------------------------------- |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 3・判断待ち 3・要 ADR 2（＝V3） |
| [findings][findings]          | レビュー指摘の処置台帳（RV-N）                     | **open は 1 件**（[RV-45][rv45]）        |
