...
develop と同じなら skip」を改める）。実装は**着手可能**。
...
- [ ] **PR のミューテーションテストを変更したファイルだけにし、`main` への PR は必ずフル run にする**（[ADR-0094][adr94]・2026-09-25 accepted・
...
| ----------------------------- | -------------------------------------------------- | ------------------------------------------------------------------ |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 1（[ADR-0094][adr94] の実装）・判断待ち 0・要 ADR 0       |
| [findings][findings]          | レビュー指摘の処置台帳（RV-N）                     | **open は 0 件** ✅                                                |
| [docs/adr][adr]               | 【accept 済み・実装済み】＋論点バックログ          | **decide 待ち 0 件** ✅・反映待ち 1 件（[ADR-0094][adr94] の実装） |
