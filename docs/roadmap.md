...
その議論から派生した **[ADR-0059][adr59]**（`field` を接頭辞なしで受ける）も `proposed`（2026-08-29）。
**2 本は互いに独立に accept できる**。
...
| 3   | **[ADR-0059][adr59] の決定**（議論 → accepted） | 0058 から派生     | —      | `field` だけ接頭辞付きという非対称。4 案（現状維持 / 両対応 / bare のみ / 型付き bare） |
| 4   | ADR-0059 の実施（accept 後・**別 PR**）         | [ADR-0059][adr59] | minor  | 公開 API の破壊的変更を含む（移行は接頭辞を消すだけ）                                   |
...
| ----------------------------- | -------------------------------------------- | ----------------------------------------------- |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 随時 / 待ち） | 着手可能 4 件（0058 / 0059 の決定 → 実施）      |
| [findings][findings]          | レビュー指摘の処置台帳（RV-N）               | open 2 件 = RV-22（契約待ち）/ RV-31            |
| [docs/adr][adr]               | 【accept 済み・実装済み】＋論点バックログ    | proposed **2 件**（0058 / 0059）／実装待ち なし |
...
- ADR 0001〜0057 accepted（0037 は 0039 で superseded）＋ **0058 / 0059 proposed**（決定待ち）・実装待ちは無し（[索引][adr]）
