...
**2026-09-25 に判断待ちが 1 件に戻った**: PR のミューテーションテストを変更したファイルだけにし、リリース PR は必ず
フル run にするか — [ADR-0094][adr94]（[ADR-0015][adr15] の「PR もフル run」と [ADR-0028][adr28] の「リリース PR は
...
- [ ] **PR のミューテーションテストを変更したファイルだけにし、リリース PR は必ずフル run にするか** — [ADR-0094][adr94]
      （2026-09-25 起票・**proposed**・stakeholder の指摘）。推奨: 機能 PR は変更した `src` のファイル（と隣のテストが変わった実装）だけを
      incremental を使わずに検査し、`test/`・依存・設定の変更はフル run に倒す。リリース PR は必ずフル run。develop への push と nightly は据え置く。
