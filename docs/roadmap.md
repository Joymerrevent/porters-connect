...
**最新は 0.22.0**（2026-09-23）＝ 数値でない文字列を `Number` として読まず `validation` で止める（RV-58）・
...
品質ゲートは全 green（**1492 tests**・coverage は perFile 100%・**mutation 100**＝`break` 100 を CI が強制・2026-09-23 時点）。
...
| V3  | **自分で進められる作業が残っていない** | 本書の「着手可能」「判断待ち」「要 ADR」がすべて 0 件 ／ [findings][findings] の open が 0 件                                              | **open 1 件**（2026-09-24・[ADR-0090][adr90] は accepted・同日実装。[RV-63][rv63] は条件付き＝実例待ち）。それまでは着手可能・判断待ち・要 ADR とも 0（2026-09-24・[RV-62][rv62] は [ADR-0089][adr89] を accepted にして同日実装・fixed）。それまでは達成（2026-09-22 に [ADR-0088][adr88] の実装が着手可能に 1 件戻ったが、同日 [#378][pr378] で完了）。それ以外も達成のまま（2026-09-21・run 2 の open 4 件＝ [RV-58][rv58]〜61 は同日にすべて処置・open 0。[RV-59][rv59] 案 (b) は「0015 をそのまま適用」で決着＝要 ADR 0）。条件付き 1 件（[RV-54][rv54] 案 (b)・実例が出たら起票。分母に数えない）。README 英語版は `1.0.0` の後へ |
...
| [findings][findings]          | レビュー指摘の処置台帳（RV-N）                     | **open は 1 件**（[RV-63][rv63] 条件付き） |
...
- 品質ゲート green・**1492 tests**／project-review プロセス＋台帳（[findings][findings]：
  **open は 1 件**（2026-09-24・[RV-63][rv63] 条件付き。[RV-62][rv62] は fixed）。RV-36 / RV-37 はどちらも 2026-09-09 に案D の評価過程で検出し、2026-09-10 に fixed。
...
- [x] 公開済み — **`@joymerrevent/porters-connect@0.22.0`**（npm latest・2026-09-23 にレジストリで確認・**8 files / 955.9 kB**）。**全 30 版**を半自動フローでリリース:
      0.1.0 → 0.1.1 → 0.2.0 → 0.2.1 → 0.3.0 → 0.4.0 → 0.5.0 → 0.6.0 → 0.6.1 → 0.6.2 → 0.7.0 → 0.8.0 → 0.9.0 → 0.10.0 → 0.11.0 → 0.12.0 → 0.12.1 → 0.13.0 → 0.14.0 → 0.15.0 → 0.15.1 → 0.16.0 → 0.17.0 → 0.18.0 → 0.19.0 → 0.19.1 → 0.20.0 → 0.20.1 → 0.21.0 → 0.22.0
...
- [x] publish ワークフロー `release.yml`（**Release 公開**で起動・**OIDC Trusted Publishing**・**NPM_TOKEN 不要**・provenance 自動）＋ npm 側の信頼登録済み（0.1.0〜0.22.0 の**全 30 版**で運用実績あり）
