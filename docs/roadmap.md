...
**いま残っているのは 2 件だけ**（2026-09-18 時点）。**V1・V2 は達成**し、ADR も findings も
待ちが無い。V3 の残りは **任意の README 英語版**と **成功指標の数値化タイミング**（PRD [§8][prd]）で、
後者は stakeholder 判断。**V5・V6 は契約待ち**なので、自分で進められる作業はほぼ尽きている
＝ここから先は「契約を取る」が最大のボトルネック。
...
| V3  | **自分で進められる作業が残っていない** | 本書の「着手可能」「判断待ち」「要 ADR」がすべて 0 件 ／ [findings][findings] の open が 0 件                                              | 着手可能 **1**（任意の README 英語版のみ）・判断待ち **1**・要 ADR 0・**open 0**                               |
...
| **判断待ち** | 残り 1 件＝**成功指標の数値化**（上記「判断待ち」節）＝ **V3**。Sales の必須は [ADR-0083][adr83] で決着 |
...
| ----------------------------- | -------------------------------------------------- | ---------------------------------------- |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 1・判断待ち 1・要 ADR 0（＝V3） |
| [findings][findings]          | レビュー指摘の処置台帳（RV-N）                     | **open は 0 件** ✅                      |
| [docs/adr][adr]               | 【accept 済み・実装済み】＋論点バックログ          | **decide 待ち・実装待ちとも 0 件** ✅    |
