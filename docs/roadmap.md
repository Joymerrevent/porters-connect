...
**2026-09-25 に判断待ちが 1 件に戻った**: リソースの公開の型を、共通の型の別名ではなく各リソースのファイルでメソッドを書き出すか —
[ADR-0100][adr100]（stakeholder の議論。データ系の API リファレンスにメソッドの説明が出ていない穴と、「どんなメソッドを持つかは
リソースごとの責任」という考え方から。公開 API は変わらない）。
...
| V3  | **自分で進められる作業が残っていない** | 本書の「着手可能」「判断待ち」「要 ADR」がすべて 0 件 ／ [findings][findings] の open が 0 件                                              | **着手可能 1 件・判断待ち 1 件**（2026-09-25・[ADR-0096][adr96] の実装待ち。[ADR-0100][adr100] は proposed。[ADR-0099][adr99] は起票・accepted・同日実装。[ADR-0098][adr98] は起票・accepted・同日実装。[ADR-0097][adr97] は起票・accepted・同日実装。[ADR-0095][adr95] は起票・accepted・同日実装）。それまでは達成に戻っていた（2026-09-25・[ADR-0094][adr94] を起票・accepted・同日実装。着手可能・判断待ち・要 ADR とも 0）。それまでは達成（2026-09-24・[ADR-0093][adr93] を起票・accepted・同日実装し、続く実践例 2 本も同日に足した。着手可能・判断待ち・要 ADR とも 0）。それまでは達成（2026-09-24・[ADR-0092][adr92] を起票・accepted・同日実装。着手可能・判断待ち・要 ADR とも 0）。それまでは達成（2026-09-24・[RV-63][rv63] を [ADR-0091][adr91] として起票・accepted・同日実装・fixed。[ADR-0090][adr90] も同日実装。着手可能・判断待ち・要 ADR・open とも 0）。それまでは着手可能・判断待ち・要 ADR とも 0（2026-09-24・[RV-62][rv62] は [ADR-0089][adr89] を accepted にして同日実装・fixed）。それまでは達成（2026-09-22 に [ADR-0088][adr88] の実装が着手可能に 1 件戻ったが、同日 [#378][pr378] で完了）。それ以外も達成のまま（2026-09-21・run 2 の open 4 件＝ [RV-58][rv58]〜61 は同日にすべて処置・open 0。[RV-59][rv59] 案 (b) は「0015 をそのまま適用」で決着＝要 ADR 0）。条件付き 1 件（[RV-54][rv54] 案 (b)・実例が出たら起票。分母に数えない）。README 英語版は `1.0.0` の後へ |
...
- [ ] **リソースの公開の型を、各リソースのファイルでメソッドを書き出すか** — [ADR-0100][adr100]（2026-09-25 起票・**proposed**・
      stakeholder の議論）。案A: データ系・マスタとも書き出し、共通の型（`DataResource` / `MasterResource`）は公開の型に使わない。
      揃っていることは型のテストで確かめる。ADR-0096 の実装の後に行う。
...
| **判断待ち** | **1 件**（[ADR-0100][adr100]・2026-09-25 起票）                                                          |
...
| ----------------------------- | -------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 1（[ADR-0096][adr96]）・判断待ち 1（[ADR-0100][adr100]）・要 ADR 0               |
| [findings][findings]          | レビュー指摘の処置台帳（RV-N）                     | **open は 0 件** ✅                                                                       |
| [docs/adr][adr]               | 【accept 済み・実装済み】＋論点バックログ          | **decide 待ち 1 件**（[ADR-0100][adr100]）・**反映待ち 1 件**（[ADR-0096][adr96] の実装） |
