...
| V3  | **自分で進められる作業が残っていない** | 本書の「着手可能」「判断待ち」「要 ADR」がすべて 0 件 ／ [findings][findings] の open が 0 件                                              | **判断待ち 1 件・open 1 件**（2026-09-24・[ADR-0090][adr90] proposed。[RV-63][rv63] は条件付き＝実例待ち）。それまでは着手可能・判断待ち・要 ADR とも 0（2026-09-24・[RV-62][rv62] は [ADR-0089][adr89] を accepted にして同日実装・fixed）。それまでは達成（2026-09-22 に [ADR-0088][adr88] の実装が着手可能に 1 件戻ったが、同日 [#378][pr378] で完了）。それ以外も達成のまま（2026-09-21・run 2 の open 4 件＝ [RV-58][rv58]〜61 は同日にすべて処置・open 0。[RV-59][rv59] 案 (b) は「0015 をそのまま適用」で決着＝要 ADR 0）。条件付き 1 件（[RV-54][rv54] 案 (b)・実例が出たら起票。分母に数えない）。README 英語版は `1.0.0` の後へ |
...
- [ ] **利用者の TypeScript の下限を仕組みで守るか** — [ADR-0090][adr90]（2026-09-24 起票・**proposed**）。
      同梱の型定義が `NoInfer`（5.4 で追加）を使い、5.4 より古い版では黙って誤った型になる。推奨は、`exports` の
      `types@<5.4` 条件で古い版を「5.4 以上が要る」と読める型定義に向け、その値を正にして README・CI と突き合わせ、
      CI で下限の版（パッチまで固定）と開発の版の 2 つで利用者のコードをコンパイルする。
...
| ----------------------------- | -------------------------------------------------- | ----------------------------------------------------- |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 0・判断待ち 1（[ADR-0090][adr90]）・要 ADR 0 |
