...
| V2  | **使い方のドキュメント完成**           | 「入門（順に読む）→ 考え方 → 目的別 HOWTO」が揃い、**読者の目的から引ける**こと。検査できる形は要 ADR で確定                               | **未着手**（構成を ADR で起票する）          |
...
- [ ] **使い方ドキュメントの構成（V2 の本体）** — [ADR-0035][adr35] は「README は最短で動かす入口・
      機能ごとの手順は `docs/guide/<topic>.md`」と決めたが、**読者の目的から引く構成にはなっていない**。
      入門の順序・考え方のページ・目的別 HOWTO の粒度・V2 の検査できる完了条件を決める（0035 を supersede
      するか拡張するかも含む）
