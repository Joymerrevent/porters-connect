...
（stakeholder の問い。はじめは `src/resources/` の中だけの案で、議論で `src/` 全体に広げた）。実装は**着手可能**で、[ADR-0096][adr96] の実装より先に行う。
...
| ----------------------------- | -------------------------------------------------- | ---------------------------------------------------------------------------------- |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 2（[ADR-0097][adr97]・[ADR-0096][adr96]）・判断待ち 0・要 ADR 0           |
| [findings][findings]          | レビュー指摘の処置台帳（RV-N）                     | **open は 0 件** ✅                                                                |
| [docs/adr][adr]               | 【accept 済み・実装済み】＋論点バックログ          | decide 待ち 0 件・**反映待ち 2 件**（[ADR-0097][adr97]・[ADR-0096][adr96] の実装） |
