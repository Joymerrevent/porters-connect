...
| V2  | **使い方のドキュメント完成**           | 「入門（順に読む）→ 考え方 → 目的別 HOWTO」が揃い、**読者の目的から引ける**こと。検査できる形は要 ADR で確定                               | 構成は [ADR-0070][adr70] で起票・**受理待ち** |
| V3  | **自分で進められる作業が残っていない** | 本書の「着手可能」「判断待ち」「要 ADR」がすべて 0 件 ／ [findings][findings] の open が 0 件                                              | 着手可能 2・判断待ち 3・要 ADR 1・**open 0**  |
...
- [ ] **使い方のドキュメント（V2）** — **要 ADR から**（下記「要 ADR」節）。現状のガイド 7 本は
      **機能単位**（oauth / read-query / custom-fields …）で機能追加のたびに増えた形で、
      **順に読む道筋が無く、目的（「〜したい」）から引けない**。Jest / Next.js の構成
      （入門 → 考え方 → 目的別 → リファレンス）に寄せて再構成し、穴を埋める。
      **サイトの構築はしない**（原稿までで、後から載せられる形にする＝[ADR-0068][adr68] の想定どおり）
...
- [ ] ✍️ **使い方ドキュメントの構成（V2 の本体）** → [ADR-0070][adr70] で**起票済み・`proposed`**（受理待ち）。
      Jest / Next.js の構成を実測して「入門（順に読む）／考え方／目的別 HOWTO／リファレンス」の 4 層を提案。
      論点は 6 つ（IA・既存 7 本の扱い・README と [ADR-0035][adr35] の関係・V2 の検査できる完了条件・
      サイト化への備え・英語版）
