...
**最新は 0.25.0**（2026-09-25）＝ 定義していないオプションのキーを弾く（[ADR-0092][adr92]）・`porters.auth.getToken()` が
期限も返す（[ADR-0093][adr93]）minor 版。**破壊的変更 2 つ**。ひとつ前の **0.24.0**（2026-09-24）＝ トークンの取り方（`tokenProvider`）と置き場所（`tokenStore`）を分ける（[ADR-0091][adr91]）・
...
**破壊的変更なし**（宣言が実際の項目と違うと読み取りがエラーになりうるので minor）。
ひとつ前の **0.21.0**（2026-09-21）＝ カスタム項目の宣言を `tenant(id, { fields })` で受ける minor 版
...
品質ゲートは全 green（**1578 tests**・coverage は perFile 100%・**mutation 100**＝`break` 100 を CI が強制・2026-09-23 時点）。
...
**2026-09-26 に判断待ちが 1 件に戻り、同日 accepted になった**: リソースの共通の仕組み（`src/resources/core/`）を `src/accessor/` に出して層を 1 段足すか、
ファイル名の決まりで factory の `create` を付けてもよいことにするか — [ADR-0101][adr101]（stakeholder の議論。[ADR-0097][adr97] の
案3a と名前の決まりを置き換える。decider が案1a ＋ 案2a を選んだ）。実装は**着手可能**で、`resources/core/` のリファクタリング（#434）の
マージ後に行う。

**2026-09-25 に判断待ちが 1 件に戻り、同日 accepted になった**: リソースの公開の型を、共通の型の別名ではなく各リソースのファイルでメソッドを書き出す —
[ADR-0100][adr100]（stakeholder の議論。データ系の API リファレンスにメソッドの説明が出ていない穴と、「どんなメソッドを持つかは
リソースごとの責任」という考え方から。公開 API は変わらない）。**同日実装**（[ADR-0096][adr96] の実装の後）。

**2026-09-25 に判断待ちが 1 件に戻り、同日 accepted になった**: 検索クエリの型からページ送り（`count` / `start`）を外し、`Paging` / `Limit` を別の型にする —
[ADR-0099][adr99]（stakeholder の問い。データ系・マスタ・Attachment・Option の `search` を「クエリ ＋ ページ送り」、`searchAll` を
「クエリ」に揃える。**公開 API の破壊的変更**）。**同日実装**（公開 API を変えないリファクタリングの PR の後に行った）。

**2026-09-25 に判断待ちが 1 件に戻り、同日 accepted になった**: PORTERS が決めた値と定義表を `src/porters/` にまとめる — [ADR-0098][adr98]
（stakeholder の提案。[ADR-0097][adr97] の実装を見て「`resources/` 直下はリソースだけにしたい」「PORTERS 側の決まりを責務で
1 か所にまとめたい」。案B＝定義表と上限・値・対応表をまとめ、`porters/` を一番下の層にする）。**同日実装**（[ADR-0096][adr96] の実装より先に行った）。

**2026-09-25 に判断待ちが 1 件に戻り、同日 accepted になった**: `src/` のモジュールを層に並べ、依存の向きを lint で守る — [ADR-0097][adr97]
（stakeholder の問い。はじめは `src/resources/` の中だけの案で、議論で `src/` 全体に広げた）。**同日実装**（[ADR-0096][adr96] の実装より先に行った）。
...
**同日実装**（[ADR-0095][adr95]・[ADR-0099][adr99] の実装の後）。

**2026-09-25 に判断待ちが 1 件に戻り、同日 accepted になった**: 複数の ID でまとめて読む `getMany(ids)` を足す — [ADR-0095][adr95]
（stakeholder の問い。`{idAlias}:or` で束ねて読み、返ってきた ID を突き合わせて前提が外れたらエラーで止める。`get` と `getMany` の
両方で `field` を受ける）。**同日実装**。

**2026-09-25 に判断待ちが 1 件に戻り、同日 accepted になった**: PR のミューテーションテストを変更したファイルだけにし、`main` への PR は必ず
フル run にするか — [ADR-0094][adr94]（[ADR-0015][adr15] の「PR もフル run」と [ADR-0028][adr28] の「リリースの流れの PR は
develop と同じなら skip」を改める）。**同日実装**。

**2026-09-24 に判断待ちが 1 件に戻り、同日 accepted になった**: `porters.auth.getToken()` が Access Token の期限も返す —
[ADR-0093][adr93]（案A＝戻り値を `IssuedToken` に。[ADR-0034][adr34] SD-6 を改める）。**同日実装**（破壊的）。

**2026-09-24 に判断待ちが 1 件に戻り、同日 accepted になった**: 構築オプションと `tenant()` のオプションで、定義していないキーを
実行時にすべて弾く — [ADR-0092][adr92]（[ADR-0087][adr87] / [ADR-0091][adr91] の `never` の型付けを改める）。**同日実装**（破壊的）。
...
「1 本にまとめる」に改めた（ADR の訂正注記）。**2026-09-23 に「クライアント」と「関数」の章を足して 7 章にした**
（同じ [#378][pr378] に含めた・ADR の訂正注記）。V2 の達成は 7 章の定義で読み替え、V3 は達成に戻る。
...
| V2  | **使い方のドキュメント完成**           | 「入門（順に読む）→ 考え方 → 目的別 HOWTO」が揃い、**読者の目的から引ける**こと。検査は [ADR-0070][adr70] の 5 つ                          | **達成**（7 章 ＋ 検査①〜⑥ ＋ README を入口に絞る ＋ `docs/usage/` への集約。[ADR-0088][adr88] で 4 層から組み直し・2026-09-22〜23・[#378][pr378]）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| V3  | **自分で進められる作業が残っていない** | 本書の「着手可能」「判断待ち」「要 ADR」がすべて 0 件 ／ [findings][findings] の open が 0 件                                              | **達成**（2026-09-26・[ADR-0101][adr101] の追記＝いまあるファイルを「1 ファイルに主な export は 1 つ」に合わせる作業を同日実装。着手可能・判断待ち・要 ADR とも 0）。それまでは着手可能 1 件（2026-09-26・[ADR-0101][adr101] の追記＝いまあるファイルを「1 ファイルに主な export は 1 つ」に合わせる。`src/accessor/` への移動は同日実装）。それまでは達成（2026-09-25・[ADR-0100][adr100] を同日実装。着手可能・判断待ち・要 ADR とも 0）。それまでは着手可能 1 件（2026-09-25・[ADR-0100][adr100] の実装待ち。[ADR-0096][adr96] は同日実装。[ADR-0099][adr99] は起票・accepted・同日実装。[ADR-0098][adr98] は起票・accepted・同日実装。[ADR-0097][adr97] は起票・accepted・同日実装。[ADR-0095][adr95] は起票・accepted・同日実装）。それまでは達成に戻っていた（2026-09-25・[ADR-0094][adr94] を起票・accepted・同日実装。着手可能・判断待ち・要 ADR とも 0）。それまでは達成（2026-09-24・[ADR-0093][adr93] を起票・accepted・同日実装し、続く実践例 2 本も同日に足した。着手可能・判断待ち・要 ADR とも 0）。それまでは達成（2026-09-24・[ADR-0092][adr92] を起票・accepted・同日実装。着手可能・判断待ち・要 ADR とも 0）。それまでは達成（2026-09-24・[RV-63][rv63] を [ADR-0091][adr91] として起票・accepted・同日実装・fixed。[ADR-0090][adr90] も同日実装。着手可能・判断待ち・要 ADR・open とも 0）。それまでは着手可能・判断待ち・要 ADR とも 0（2026-09-24・[RV-62][rv62] は [ADR-0089][adr89] を accepted にして同日実装・fixed）。それまでは達成（2026-09-22 に [ADR-0088][adr88] の実装が着手可能に 1 件戻ったが、同日 [#378][pr378] で完了）。それ以外も達成のまま（2026-09-21・run 2 の open 4 件＝ [RV-58][rv58]〜61 は同日にすべて処置・open 0。[RV-59][rv59] 案 (b) は「0015 をそのまま適用」で決着＝要 ADR 0）。条件付き 1 件（[RV-54][rv54] 案 (b)・実例が出たら起票。分母に数えない）。README 英語版は `1.0.0` の後へ |
| V4  | **残る未確認はライブ検証だけ**         | **`pnpm check:lv` が緑**＝コードの `VERIFY(live)` と [live-verification][lv] のエントリが双方向で対応する（[RV-50][rv50]）                 | **達成**（検査が CI に載った。未確認 30 ／ 確定 1 ／ 解消 2）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| V5  | **契約後、その検証を全件確定させる**   | [live-verification][lv] の **`未確認` が 0 件**（`確定` と `解消` はどちらも終端 — [状態の意味][lvstate]）                                 | **未確認 30 件**（契約待ち）。`確定` 1・`解消` 2                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
- [x] ✅ **リソースの共通の仕組みを `src/accessor/` に移す**（[ADR-0101][adr101]・2026-09-26 accepted・同日実装・stakeholder の議論）。
      決定: `src/resources/core/` を `src/accessor/` に移し、層の表に `accessor` を 1 段足す（`core/` のための別の規則は無くす）。
      ファイル名の `create` は付けても付けなくてもよい（いまのファイル名は変えない）。**#434（`resources/core/` のリファクタリング）の
      マージ後に着手**（ほぼ全ファイルの import を書き換えるため）。
- [x] ✅ **いまあるファイルを「1 ファイルに主な export は 1 つ」に合わせる**（[ADR-0101][adr101] の追記・2026-09-26・同日実装・stakeholder の議論）。
...
- [x] ✅ **検索クエリの型からページ送りを外し、`Paging` / `Limit` を足す**（[ADR-0099][adr99]・2026-09-25 accepted・同日実装・stakeholder の問い）。
      ...
      `AttachmentWalkQuery` は無くす。破壊的変更（changeset に移行の案内）。公開 API を変えないリファクタリングの PR の後に着手。
- [x] ✅ **PORTERS が決めた値と定義表を `src/porters/` にまとめる**（[ADR-0098][adr98]・2026-09-25 accepted・同日実装・stakeholder の提案）。
      決定: resource-list / field-type と、出典に書いてある上限・値・対応表（`DataType` の一覧を含む）を `src/porters/` に移し、
      何も import しない一番下の層にする。ライブラリの判断と、値を使うコードは今の場所に残す。`resource-list` は向きを逆にする。
      公開 API は変えない。**[ADR-0096][adr96] の実装より先に着手**。
- [x] ✅ **`src/` のモジュールを層に並べ、依存の向きを lint で守る**（[ADR-0097][adr97]・2026-09-25 accepted・同日実装・stakeholder の問い）。
...
- [x] ✅ **複数の ID でまとめて読む `getMany(ids)` を足す**（[ADR-0095][adr95]・2026-09-25 accepted・同日実装・stakeholder の問い）。
      ...
      エラーで止める。戻り値は渡した順の `(T | undefined)[]`。Attachment とマスタには付けない。live-verification に LV-33（`P_Id:or`）を足した。
- [x] ✅ **読み取りの戻り値の型を `field` で絞る**（[ADR-0096][adr96]・2026-09-25 accepted・同日実装・[ADR-0095][adr95] から分けた）。
      決定: `search` / `searchAll` / `get` / `getMany` のすべてで、`field`（と `expand` / `image`）で要求していない項目のキーを型から外す
      （読んだ項目は省略可能のまま）。型の破壊的変更。**[ADR-0095][adr95] の実装の後に着手**。
- [x] ✅ **リソースの公開の型を、各リソースのファイルでメソッドを書き出す**（[ADR-0100][adr100]・2026-09-25 accepted・同日実装・stakeholder の議論）。
      決定: データ系・マスタとも書き出し、`DataResource` / `MasterResource` は公開の型に使わない。引数や戻り値の型は項目の一覧から導く
      （ADR-0019 のまま）。データ系 12 種が揃っていることを型のテストで確かめる。**[ADR-0096][adr96] の実装の後に着手**。
- [x] ✅ **PR のミューテーションテストを変更したファイルだけにし、`main` への PR は必ずフル run にする**（[ADR-0094][adr94]・2026-09-25 accepted・同日実装・
      stakeholder の指摘）。決定: 機能 PR は変更した `src` のファイル（と隣のテストが変わった実装）だけを
      incremental を使わずに検査し、`test/`・依存・設定の変更はフル run に倒す。`main` への PR は必ずフル run（ブランチ名で見分けない。back-merge は
      版番号と文書だけなので自然に skip）。develop への push と nightly は据え置く。
- [x] ✅ **実践例を 2 本足す: 「トークンを DB に保存する（`tokenStore`・Drizzle ORM）」と「中央のサービスから `tokenProvider` で受け取る」**
      （2026-09-24・stakeholder の提案・同日完了）。中央のサービスは `porters.auth.getToken()` の期限つきの値をアプリへ渡す。
- [x] ✅ **`porters.auth.getToken()` が Access Token の期限も返す**（[ADR-0093][adr93]・2026-09-24 accepted・同日実装・stakeholder との議論）。
      決定: 戻り値を `IssuedToken`（`{ token, expiresAt? }`）に変える（破壊的）。Refresh Token は返さない。
- [x] ✅ **構築オプションと `tenant()` のオプションで、定義していないキーを実行時にすべて弾く**（[ADR-0092][adr92]・2026-09-24 accepted・同日実装・
      stakeholder の指摘）。決定: `PortersClientOptions` から `auth?: never` / `fields?: never` を外し、構築時と `tenant()` の呼び出し時に、
      定義していないキー（値が `undefined` のものを除く）を `PortersConfigError` で止める。廃止した名前を特別に案内しない。
      許可するキーの一覧は型と `satisfies` で突き合わせる（破壊的）。
- [x] ✅ **トークンの取得と保存を別々に受け取り、管理は `PortersClient` が受け持つ**（[ADR-0091][adr91]・2026-09-24 accepted・同日実装・
      起票元 [RV-63][rv63]。条件付きだったが stakeholder が条件を待たずに進めると決めた）。決定:
      ...
      丸ごと自前で書く入口は無くす（破壊的）。`StoredTokens` は `accessToken` と省略可能な `refreshToken` に分け、それぞれ `{ token, expiresAt? }` にする。
- [x] ✅ **利用者の TypeScript の下限（5.4）を仕組みで守る**（[ADR-0090][adr90]・2026-09-24 accepted）— **完了**（同日。`check:ts-floor` を CI に載せた）。
      `exports` / `typesVersions` に `types@<5.4` の向け先（「5.4 以上が要る」と読める型定義）を足し、その値を正にして
      `check:release` が README・導入のページ・CI の版と突き合わせる。CI で 5.4.5 と開発の版で利用者のコードをコンパイルし、
      5.3 で明示のエラーが出ることも確かめる。解決方式（`bundler` / `node16` / `node`）ごとの効き方を確かめる。
- [x] ✅ **カスタム項目を宣言で明示したときだけ `create` の必須にする**（[ADR-0089][adr89]・2026-09-24 accepted・起票元 [RV-62][rv62]）— **完了**（同日）。
      `f.number({ required: true })` で `create` / `createMany` の入力型の必須にする（`update` は任意のまま・型だけで実行時の検査はしない）。
      `readCustomCatalog` が `P_Required` を読み、`generateFieldDecls` は `P_Required=1` に `{ required: true }` を出し、
      `verifyFields` は必須の食い違いを新しい区分で報告する（`ok` は倒さない）。ライブ検証に「Write API は `P_Required` を
      強制するか」（LV-32）をコードの印と一緒に足し、ガイド「カスタム項目」の必須の説明を書き直した。
- [x] ✅ **使い方ドキュメントに「クライアント」と「関数」の章を足す**（[ADR-0088][adr88] の訂正注記・2026-09-23）— **完了**（同日・[#378][pr378] に含めた。起票時の「クライアントと関数」1 章から、実装中に関数を別章に分けてリソース別の後に置いた）。
      ...
      主題別の `auth.md` / `tenant.md` からメソッドの表を移し、目次を 7 章にし、検査①③④⑤の定数に `client/` を足して
...
- [x] ✅ **リソースの共通の仕組みを `src/accessor/` に出し、ファイル名の `create` を任意にするか** — [ADR-0101][adr101] で決着
      （2026-09-26 起票・同日 accepted・stakeholder の議論）。実装は**着手可能**（上記）。案1a: `src/resources/core/` を `src/accessor/` に移し、層の表に 1 段足して
      `core/` のための別の規則を無くす。案2a: ファイル名は主な export の名前のまま、factory の `create` は付けても付けなくてもよい。
      [ADR-0097][adr97] の案3a と名前の決まりを置き換える。
...
- **[LV-16][lv] に `VERIFY(live)` を設置済み**（いまは `src/accessor/apply-expand.ts` の `expandEntry`）。
  外れても直すのは要求文字列だけ。
...
| 待ちの種類   | 中身                                                                                                     |
| ------------ | -------------------------------------------------------------------------------------------------------- |
| **契約待ち** | ライブ検証の **`未確認` 30 件**（[live-verification][lv]）。0.x のブロッカーではないが **`1.0.0` の V5** |
| **需要待ち** | フェイクサーバー **フェーズ7**（package 昇格・配布。[実装計画][fake-plan]・stakeholder 2026-08-09）      |
| **判断待ち** | **0 件** ✅（[ADR-0101][adr101] は 2026-09-26 に accepted）                                              |
...
| ----------------------------- | -------------------------------------------------- | --------------------------------------- |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 0・判断待ち 0・要 ADR 0        |
| [findings][findings]          | レビュー指摘の処置台帳（RV-N）                     | **open は 0 件** ✅                     |
| [docs/adr][adr]               | 【accept 済み・実装済み】＋論点バックログ          | decide 待ち 0 件・反映待ち 0 件         |
| [live-verification][lv]       | 契約取得後に実機確認する仮定（LV-N）               | **未確認 30 件**（契約待ち）            |
...
- 品質ゲート green・**1578 tests**／project-review プロセス＋台帳（[findings][findings]：
  **open は 0 件**（2026-09-24・[RV-62][rv62] / [RV-63][rv63] とも fixed）。RV-36 / RV-37 はどちらも 2026-09-09 に案D の評価過程で検出し、2026-09-10 に fixed。
...
- [x] 公開済み — **`@joymerrevent/porters-connect@0.25.0`**（npm latest・2026-09-25 にレジストリで確認・**10 files / 1067.7 kB**）。**全 33 版**を半自動フローでリリース:
      0.1.0 → 0.1.1 → 0.2.0 → 0.2.1 → 0.3.0 → 0.4.0 → 0.5.0 → 0.6.0 → 0.6.1 → 0.6.2 → 0.7.0 → 0.8.0 → 0.9.0 → 0.10.0 → 0.11.0 → 0.12.0 → 0.12.1 → 0.13.0 → 0.14.0 → 0.15.0 → 0.15.1 → 0.16.0 → 0.17.0 → 0.18.0 → 0.19.0 → 0.19.1 → 0.20.0 → 0.20.1 → 0.21.0 → 0.22.0 → 0.23.0 → 0.24.0 → 0.25.0
      ...
      **0.25.0 で定義していないオプションの拒否（[ADR-0092][adr92]・破壊的）＋ `getToken()` が期限も返す（[ADR-0093][adr93]・破壊的）**）。
...
- [x] publish ワークフロー `release.yml`（**Release 公開**で起動・**OIDC Trusted Publishing**・**NPM_TOKEN 不要**・provenance 自動）＋ npm 側の信頼登録済み（0.1.0〜0.25.0 の**全 33 版**で運用実績あり）
...
実 PORTERS 契約が無いと確定できない仮定は [live-verification][lv]（**LV-1〜33**・うち未確認 30）に集約。リリースのブロッカーではないが、
...
[adr98]: adr/0098-porters-rules-folder.md
[adr99]: adr/0099-search-query-without-paging.md
[adr100]: adr/0100-expand-resource-types.md
[adr101]: adr/0101-accessor-layer-and-file-names.md
...
[adr28]: adr/0028-ci-path-filtering.md
