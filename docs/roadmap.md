...
- [ ] **Sales の `create` 必須を ADR にするか** — **起票済み**（2026-09-18・[ADR-0083][adr83]・`proposed`）。
      問いは「`※`（条件付き必須）を公開の入力型で必須にするか」に整理した。推奨は案A（`●` だけを
      型の必須にする＝現状の明文化）。**決定は decider 待ち**で、accept されれば実装の変更は無い。
...
| **判断待ち** | 上記「判断待ち」節を参照（Sales の必須＝[ADR-0083][adr83] で起票済み・decider 待ち／成功指標の数値化）＝ **V3** |
...
| [docs/adr][adr]               | 【accept 済み・実装済み】＋論点バックログ          | **decide 待ちが 1 件**（[ADR-0083][adr83]） |
