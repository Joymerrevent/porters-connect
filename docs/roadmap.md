...
- [ ] **実践例を 2 本足す: 「トークンを DB に保存する（`tokenStore`・Drizzle ORM）」と「中央のサービスから `tokenProvider` で受け取る」**
      （2026-09-24・stakeholder の提案）。中央のサービスは `porters.auth.getToken()` の期限つきの値をアプリへ渡す。
...
| ----------------------------- | -------------------------------------------------- | ----------------------------------------------- |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 1（実践例 2 本）・判断待ち 0・要 ADR 0 |
