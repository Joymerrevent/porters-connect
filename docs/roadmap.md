...
**2026-09-26 に判断待ちが 1 件に戻った**: リソースの共通の仕組み（`src/resources/core/`）を `src/accessor/` に出して層を 1 段足すか、
ファイル名の決まりで factory の `create` を付けてもよいことにするか — [ADR-0101][adr101]（stakeholder の議論。[ADR-0097][adr97] の
案3a と名前の決まりを置き換える案。**proposed**）。実装は accepted の後、`resources/core/` のリファクタリング（#434）のマージ後に行う。
...
- [ ] **リソースの共通の仕組みを `src/accessor/` に出し、ファイル名の `create` を任意にするか** — [ADR-0101][adr101]
      （2026-09-26 起票・**proposed**・stakeholder の議論）。案1a: `src/resources/core/` を `src/accessor/` に移し、層の表に 1 段足して
...
| **判断待ち** | **1 件**（[ADR-0101][adr101]・2026-09-26 起票）                                                          |
...
| ----------------------------- | -------------------------------------------------- | ------------------------------------------------------ |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 0・判断待ち 1（[ADR-0101][adr101]）・要 ADR 0 |
| [findings][findings]          | レビュー指摘の処置台帳（RV-N）                     | **open は 0 件** ✅                                    |
| [docs/adr][adr]               | 【accept 済み・実装済み】＋論点バックログ          | decide 待ち 1 件（[ADR-0101][adr101]）・反映待ち 0 件  |
