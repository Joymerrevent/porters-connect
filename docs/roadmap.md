...
| V3  | **自分で進められる作業が残っていない** | 本書の「着手可能」「判断待ち」「要 ADR」がすべて 0 件 ／ [findings][findings] の open が 0 件                                              | 着手可能 2・判断待ち 3・要 ADR 3・**open 1**（[RV-46][rv46]）                                                             |
...
- [ ] **既定 30 秒のタイムアウトを変えられるようにするか**（[RV-46][rv46]・open）— 既定の transport は
      1 リクエスト 30 秒で打ち切るが、`createFetchTransport` を公開していないので**公開 API から
      変えられない**。効くのは大きな添付（1 ファイル 10MB まで）。`createThrottle` を公開した
      [ADR-0073][adr73] と同じ形の設定なので、揃えるなら公開 API の追加＝要 ADR
...
| ----------------------------- | -------------------------------------------------- | ---------------------------------------- |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 2・判断待ち 3・要 ADR 3（＝V3） |
| [findings][findings]          | レビュー指摘の処置台帳（RV-N）                     | **open は 1 件**（[RV-46][rv46]）        |
