...
Department は同日に実装して **V1 は再び達成**、時分型は同日に [ADR-0086][adr86] を起票（判断待ち 1）:
...
| V3  | **自分で進められる作業が残っていない** | 本書の「着手可能」「判断待ち」「要 ADR」がすべて 0 件 ／ [findings][findings] の open が 0 件                                              | 着手可能 0・**判断待ち 1**（[ADR-0086][adr86] 時分型・proposed）・要 ADR 0・open 0（README 英語版は `1.0.0` の後へ）                                                       |
...
- [ ] **DateTime の時分型（FT-12 の第 2 モード）をどう受けるか** — [ADR-0086][adr86] を **proposed で起票**
      （2026-09-20）。推奨は「`f.time()` で宣言し `"HH:mm"`（00:00〜47:59）で入出力・1970 基準日は内側に
      閉じる・`time()` 項目に基準日以外が来たら validation エラー・`DataType` は 17 のまま」。
      論点は 5 つ（区別する場所／値の型／誤宣言の倒れ方／condition・order／内部ラベル）。
      decider が選べば実装 PR へ。**0.x のブロッカーではない**。
...
| **判断待ち** | **1 件** — [ADR-0086][adr86]（時分型・proposed・2026-09-20 起票）                                        |
...
| ----------------------------- | -------------------------------------------------- | -------------------------------------------------- |
| **本書**（roadmap）           | **次に何をやるか**（着手可能 / 判断待ち / 要 ADR） | 着手可能 0・判断待ち 1（ADR-0086）・要 ADR 0       |
| [findings][findings]          | レビュー指摘の処置台帳（RV-N）                     | **open は 0 件** ✅                                |
| [docs/adr][adr]               | 【accept 済み・実装済み】＋論点バックログ          | **decide 待ち 1**（[ADR-0086][adr86]）・反映待ち 0 |
