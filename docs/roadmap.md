...
| #   | やること                                            | 根拠                       | semver | 備考                                           |
| --- | --------------------------------------------------- | -------------------------- | ------ | ---------------------------------------------- |
| 1   | **[ADR-0051][adr51] の決定**（Read 応答の同定条件） | [findings][findings] RV-20 | minor  | `proposed`・推奨は案B。実装は accept 後・別 PR |
...
- **0.7.0 を公開済み**（2026-08-13）。ADR-0044・0045・0046・0047・0048・0049・0050 の 7 本が世に出た
  （RV-13/14/15/17/19/21 は `fixed`）。**accept 済みで実装待ちの ADR は無い**。
- 1 は **RV-20**（HTTP 200 ＋ 非 PORTERS ボディが「空ページ」として通る）の是正。
  [ADR-0051][adr51] を `proposed` で起票済み（2026-08-13）。**推奨は案B**＝`<Code>` の**存在**を
  envelope の要件にする（Read のみ）。実測では**書き換えが要る既存テストは 1 件だけ**で、
  ルート要素名の検証（案A / 案D）は名前の正が未確認のため LV-14 の確認後に段階的に足す。**decider の決定待ち**。
...
| [findings][findings]          | レビュー指摘の処置台帳（RV-N）               | open 2 件 = RV-20（[0051][adr51] 決定待ち）/ 22（契約待ち） |
| [docs/adr][adr]               | 【議論中】＋【実装待ち】＋論点バックログ     | proposed 1 件（[0051][adr51]）／実装待ち **なし**           |
...
- ADR 0001〜0050 accepted（0037 は 0039 で superseded）・**[0051][adr51] のみ `proposed`**。
  **実装待ちは無し**（[索引][adr]）
