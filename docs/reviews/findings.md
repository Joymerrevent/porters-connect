...
ID は不変・エントリは消さない。確定したら「状態」を更新します。
...
- **状態**: fixed — `search`/`searchAll`/`get` が field 省略時にカタログ導出の既定 field を送る（User は4サブ展開・Reference は ID）。`field:[]` で主キーのみにオプトアウト。README/JSDoc に明記。ADR-0020 accepted（案A+2a+3a+軸4・透明化）
...
- **状態**: fixed — 既定 field 注入で fieldless search が全項目要求になり ALL fixture が現実と整合。既定 field 送信を pin するテストを `resource.test.ts` / `candidate.test.ts` に追加
...
- **状態**: fixed — rateLimit を予約として明示（型はコメント、README 整合）。強制切断は network 継続
...
- **状態**: fixed — publishConfig.access=public ＋ npm メタデータ補完
