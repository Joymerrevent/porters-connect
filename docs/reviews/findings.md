...
- **状態**: open
- **処置**: [ADR-0049][adr49] を **`proposed` で起票**（2026-08-12・**推奨は案C**）。3 案の比較・案C で必要な
  判定式の調整 3 点（両側小文字化／空文字の明示チェック／`pathname === ""`）・通る 9 例と弾く 14 例の実測・
  semver（patch＝受け入れを広げるだけ）・リリースとの関係まで ADR 側に記載。**decider の決定待ち**
