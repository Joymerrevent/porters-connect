...
| RV-10 | 🟡     | アーキテクチャ / DX       | open  |
| RV-11 | 🟡     | 認証                      | open  |
| RV-12 | 🟢     | ドキュメント              | open  |

> RV-10〜12 は横断監査（2026-06-22）で検出したドリフト群。受け入れ済み ADR が定めた v1 公開面の**未実装サーフェス**（OAuth `porters.auth.*` / Read クエリ `order`・`keywords`・`itemstate` / `tenant(id)`＋per-call `partition` / 200 件一括書き込み）は finding 化せず [ADR-0033][adr33] 案F（先行フェーズ）で扱う。
