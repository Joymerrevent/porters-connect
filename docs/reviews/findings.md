...
- **状態**: deferred
- **処置（deferred）**: 0.3.0 のブロッカーにしない。透過既定の自動再取得（refresh 不可→ `code_direct` 再取得）はブラウザ不要で**実用上は妥当な挙動**のため、doc 側（R-1／ADR-0007 受け入れ基準）を実装に合わせる方向で是正する。ADR amend を伴うため案F-3 近辺で扱う
