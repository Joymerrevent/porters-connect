...
**3 回目の追加の修正（2026-09-26・`09c1276`）。** 3 回目の再レビューで、Code 1000・表に無いコードが HTTP 4xx に載ると「書き込まれていない」になると分かった。HTTP のステータスで決めるのは PORTERS の Result Code が無いときだけにした。本物の requester と readResponse を通した全組み合わせ（Code 26 種 × HTTP 12 通りほか）で、書き込まれた可能性がある失敗を「書き込まれていない」と書くものが無いことを確かめた。

**整理（2026-09-26・`7adfc34`）。** 判定を `knownNotWritten` に絞ったあと、最初に足した `isUnknownOutcome`（`requester.ts`）の印は判定に効かなくなっていた（CI のミューテーションで分かった）ので消した。動きは変わらない。
