...
- **素通しに揃える** — 日時系の検査も外す。✗ 危険側: `isoToPortersDate` は
  `"2026/09/09"`（PORTERS 形式をそのまま渡した典型的な取り違え）を弾く側なので、
  外すと**黙って別の日付として送られる**。
- **日時系に揃えて他の型も検査する** — ガイドの「値レベルの検証はしません」を書き換えることになる。
  ロードマップの但し書き（**契約前に厳しくしすぎない**＝サーバーが受ける値を手前で落とすと危険側）が効く。
- **日時系だけ現状維持し、`validation` に寄せるだけ** — 検証範囲は増やさず、エラー型だけ揃える。
  ガイドには「日時だけは形式を検査する」と正確に書く。**最小で、上の但し書きにも反しない。**
