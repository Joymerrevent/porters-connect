...
1. **未登録だった仮定を起票**（(2) の解消）— [LV-27][lv]（複数人を持てる `User` 項目の応答形）・
   [LV-28][lv]（Sales の `※` 依存連鎖の実挙動）・[LV-29][lv]（`System[Department]` は書けるか）。
   LV-27 は「decoder が先頭だけ読む＝2 人目以降を黙って捨てる」ので、倒れ方が安全側でない点まで
   確認方法に書いた。
2. **番号の無かったマーカー 4 件に番号を入れた** — `phase.ts` は既存の [LV-17][lv] を指していたので
   番号だけ、他 3 件は上記の新エントリへ。
3. **マーカーが無かった 3 件に置いた**（(1) の解消）— `client.ts` の `buildScope`（LV-13）／
   `user.ts` の `DEFAULT_FIELDS`（LV-18）／`throttle.ts` の `sharedThrottleFor`（LV-23）。
   LV-13 は当初 `tenant` の**公開 JSDoc** に書いたが、既存の JSDoc と二重になって
   生成リファレンスから既存の説明が消えるため、内部の `buildScope` 側へ移した。
4. **検査を足した**（(3) の解消）— `scripts/check-live-verification.mjs` を `pnpm check` に載せた
   （`check:lv`）。CI のコメントにも 1 行足してある。

あわせて [live-verification][lv] に**状態の意味**（`未確認` / `確定` / `解消`）を定義した
（[RV-52][rv52] と共通の下ごしらえ）。検査が「`未確認` だけマーカーを要求する」ために、
3 値の区別が機械可読である必要があったため。

## 検証

**指摘した 3 つの穴が、いま検査で落ちることを確かめた**（`scripts/check-live-verification.test.mjs`）。
受理側だけでなく**棄却側を試している**のが要点で、検査が「何も言わないだけ」で通っていないかは
落ちることを見ないと分からない:

| 穴                                        | 検査の反応                                      |
| ----------------------------------------- | ----------------------------------------------- |
| `VERIFY(live)` に番号が無い（(2) の原因） | `src/a.ts:1: VERIFY(live) に LV-N がありません` |
| 存在しない `LV-N` を参照                  | `LV-99 を参照していますが … にありません`       |
| `未確認` なのにマーカーが無い（(1)）      | `LV-2 は「未確認」ですが、コードのどこからも…`  |
| `確定` / `解消` にマーカーが無い          | **通す**（外すのが正しい）                      |
| 状態が語彙外                              | `「済」は語彙外です`                            |
| 表と見出しのズレ                          | 両方向で落ちる                                  |
| 表が読めない（検査の空振り）              | その 1 件だけ報告して打ち切る                   |

「どこまでが 1 つのマーカーか」は**行数で切らずコメント塊で切る**ことも 4 ケースで固定した
（行数だと次の塊の番号を自分のものと誤認できる）。

実測（本 run 時点）: マーカー **27 件すべてに番号あり**・`未確認` **26 件すべてに参照あり**。
`pnpm check:lv` は `LV とコードの対応は取れています（未確認 26 ／ 確定 1 ／ 解消 2）` を出す。
これで [roadmap][rm] の **V4 は機械的に評価できる条件**になった（従来は実行すると false）。

品質ゲートは全 green（**1279 tests**・coverage perFile 100/99.21/100/100）。

[rm]: ../../roadmap.md
[rv52]: 0052-lv-gate-definition-unsatisfiable.md
