...
**実施（2026-09-27・fix/accessor-low-review・`30cfcc3`）。** 空文字や空白だけのキーワードを、送る前に拒否する。長さは UTF-16 の単位で数えるままにし、PORTERS の数え方は LV-36 に記録した（PORTERS が文字または UTF-16 で数えるなら長く見積もる側。バイトで数えるなら日本語は上限を超えて送られ、PORTERS の 400 で止まる）。
