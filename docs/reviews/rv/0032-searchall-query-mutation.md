...
(d) **1 度だけ直列化して、以降はページングだけ差し替える**（stakeholder 提案 2026-08-31・**推奨**）。
...
- **入れ子の書き換えも塞がる**のがこの案の要点。呼び出し側のオブジェクトを保持せず、
  **直列化した結果**を保持するので、`q.condition.P_Name.part` を書き換えられても後続ページに影響しない。

**(a) の浅いコピーでは塞がらない**ことも確かめた。`search` は毎ページ `readUrl` を呼び、
**その時点で** `q.condition` を直列化する。`{ ...query }` は浅いので `condition` の中身は
呼び出し側のオブジェクトのままであり、top-level の差し替え（`q.condition = {...}`）しか防げない。
...
**マスタ Read（`field` / `partition` / `user`）も同じ形**なので、直すならまとめて揃える。

### 先に挙げていた案（(d) に劣る）

- (a) 入口で浅いスナップショットを取る — 上記のとおり**入れ子は塞がらない**。
  塞がったように見えて塞がっていないので、単独では採らない。
- (b) 契約として明記するだけ（JSDoc ＋ Read クエリ ガイド）— 実装を変えない。(d) が入るなら不要。
- (c) (a) ＋ (b) — (a) の穴が残るため、(d) の下位互換。
