...
**残りを実施（2026-09-26・fix/http-auth-low-review・`55251e4`）。** 手元を入れ替えた回数を持ち、取り直しの途中で `clear()` が呼ばれていたら、取れたトークンを手元にも保存先にも戻さない。走っている取り直しを待つリクエストには、`clear()` の後に来たものも含めて、取れたトークンを返す。保存先への書き込みを待つ間に `clear()` が走る場合は残っている（RV-142）。
