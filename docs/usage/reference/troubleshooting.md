...
| `new PortersClient(...)` がその場でエラーになる                                          | オプションの形                               | `config`        | `tokenProvider` は `acquire()` を持つオブジェクトで渡す／トークンの取り方を渡すオプションは `tokenProvider` だけ（`auth` は無い）／カスタム項目の宣言は `tenant(id, { fields })` へ |
...
| 症状                                                                  | たいてい原因                                                                                                          |
| --------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| 構築した瞬間にエラーになる                                            | `hostname` に `https://` ・パス・ポートが入っている（[インストール][install]）／`tokenProvider` に `acquire()` が無い |
