...
- **サードパーティアプリ**: 第三者が開発するアプリ。**このライブラリの利用者は、ほとんどの場合これに当たる**（外部媒体エントリーの連携等）。
...
- **API アクセス数**: request 回数（**認証 request も含む**）。実装方式で大きく変わる（毎表示で取得＝多 / 定期取得して自社 DB 保持＝少）。
  → 課金・レート対策には、キャッシュ・差分取得でアクセス数を減らす（[gotchas.md][gotchas-md]）。
...
メニュー名は変更可。エディションで既定呼称が異なる（ライブラリの型名がどの画面に当たるかを引くときの参考）。
