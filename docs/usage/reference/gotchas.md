...
- **課金はアクセス数ベース**。クローリングや**ループの暴走がそのまま課金**につながる。キャッシュ・差分取得で無駄打ちを減らす（[毎日の差分同期][sync-batch]）。
...
- 契約環境が無い間も、モック transport とフェイクサーバーで**ライブラリを通るコードは検証できる**（[契約なしでテストする][test-without-contract]）。
