...
成功は HTTP 200。レスポンスは **Read とは形が違い**、ルートに `Total` / `Count` / `Start` 属性や
ルート直下の `<Code>` は**無い**。`<Item>` ごとに採番/更新された `<Id>` と**個別の `<Code>`**（結果コード）が返る
（送信した `<Item>` と同順・同数。一括 200 件の各件が独立に成否を持つ）。
...
- `<Id>`=新規登録/更新された対象レコードの ID。`<Code>`=その件の Result Code（`0`=成功。一覧は [result-codes][result-codes]）。
- エラー時は HTTP 200 以外、または該当 `<Item>` の `<Code>` が 0 以外。
- ※ 出典の Candidate サンプルは `<Id>10001<Id>`（終了タグのスラッシュ欠落）だが**記事側の誤記**。正しくは `<Id>…</Id>`。
...
- 更新時、指定する Phase は**現在の最新 Phase に対して「最新」となる条件**を満たす必要がある:
  ...
  2. （日付が同じ場合）フェーズ選択肢の並び順がより大きいこと（互換のための規則で、将来は保証されない。原則は日付を新しくする）
...
[authentication-md]: ../authentication-api/README.md
