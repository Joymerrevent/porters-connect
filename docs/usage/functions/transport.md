...
| `createFetchTransport(options?)`         | 既定の HTTP 送信を作る。`timeoutMs` でタイムアウトを延ばす（大きな添付を送るとき。[Attachment][r-attachment]）・縮める、`fetchImpl` で `fetch` を差し替える | 同期 throw（`timeoutMs` が正の整数でない・上限を超える） |
