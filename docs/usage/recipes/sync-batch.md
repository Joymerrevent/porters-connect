...
| 機能                                 | 何に使うか                                           |
| ------------------------------------ | ---------------------------------------------------- |
| `searchAll` ＋ `P_UpdateDate` の条件 | 前回の実行時刻以降に更新されたレコードだけを全件辿る |
| `createMany` / `updateMany`          | 200 件・約 15000 文字での自動分割と、件ごとの成否    |
| `BulkWriteResult`                    | 部分成功を見て、起点を進めるか決める                 |
| 内蔵スロットル ／ `createThrottle`   | 1 分あたりの上限を守る。バッチだけ枠を分けることも   |
...
更新日時で絞ります。`searchAll` がページングを引き受けるので、件数を気にせず処理できます。
...
毎日・全件を読むぶん、項目を絞るかどうかで転送量とリクエスト長が大きく変わります（[検索][search-records]）。

#### 取りこぼしを防ぐ 3 点

- **次回の起点は「今回の開始時刻」にする。** 処理中にも更新は起きます。終了時刻を起点にすると、
  処理中の更新を取りこぼします。
- **前回の範囲と重なってよい。** `ge` は「以上」なので、前回と同じレコードをもう一度拾うことがあります。
  **取り込み側を冪等に**しておけば実害はありません（取りこぼしより安全です）。
- **削除は差分で追えません。** PORTERS の API に削除はなく、画面で消されたものは更新として
  返ってきません。必要なら `itemstate` で別途読みます（[削除と削除済みデータ][no-delete]）。
...
- **`create` は再送しないでください。** 非冪等なので、届いていたら 2 件できます。しかも
  **消せません**。ライブラリも、届いたか分からない `create` は自動では再送しません（再送するのは、
  送る前に失敗したときや、PORTERS が「処理していない」と返したとき（Code `9`）のように、届いていないと分かる場合だけです）。
- **`update` は同じ内容なら再送できます。** 冪等なので、迷ったらこちらを選びます。
- **レート超過とネットワーク断は待って再送する価値があります。** ライブラリがスロットリングと
  リトライをしますが、それでも回復しないときはバッチ側の判断です。
...
**バケットは接続先（ホストとポートの組）ごと**です<!-- 根拠: ADR-0073 -->。同じ PORTERS を向くクライアントをいくつ
作っても、合計が上限に収まります。バッチと Web アプリを同じプロセスで動かしても同じです。

**別プロセスで動かす場合は違います。** 日次バッチを Web アプリとは別のプロセスで動かすと、
2 つのバケットが並びます。合計で守りたいなら `Throttle` を自分で実装して渡します
（`take(write: boolean): Promise<void>` の 1 メソッドなので、Redis など共有ストアで実装すればプロセス間で合計を守れます）。
