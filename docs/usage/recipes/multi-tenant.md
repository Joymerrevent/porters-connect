...
アプリが大きくなると、`tenant(id, { fields })` のスコープを**引数に取る関数**を切り出したくなります。
...
書くことはできる（`U_` / `A_` で始まる alias は常に要求できます）のに、受け取った値の型には
出ない、ということです。
...
それでも**宣言はプロジェクトに 1 か所置いて export する**のが簡単です
（`generateFieldDecls` の出力先がその置き場になります）。テナントごとに宣言が違うなら、
宣言とそれを受ける関数を同じモジュールにまとめます。
...
- 実践例: [毎日の差分同期][sync-batch]（バッチだけ枠を分ける）／[トークンを DB に保存する][token-store-db]（テナントごとのキー）
- ほかの目的から探す: [目次][index]
