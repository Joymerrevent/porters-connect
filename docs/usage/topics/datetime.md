...
※ `Date` / `Age` に、時刻とゾーンのある ISO（`2026-09-10T23:00:00-09:00` など）を渡すと、UTC に直した日付
（この例では `2026/09/11`）として送ります。暦に無い日付（`2026-02-30`）や、日付の後ろに別の文字が続く値は、
送る前に `PortersConfigError` になります。
