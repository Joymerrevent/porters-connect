...
※ `Date` / `Age` には、日付だけ（`2026-09-11`）を渡してください。時刻の付いた値は、UTC（末尾が `Z` または `+00:00`）のもの
（`2026-09-11T00:00:00Z`）だけを受け、その日付を送ります。日本時間などほかのゾーンの値（`2026-09-11T00:00:00+09:00`）は、
UTC に直すと前日になってしまうので受けずに、送る前に `PortersConfigError` になります。暦に無い日付（`2026-02-30`）や、
日付の後ろに別の文字が続く値も同じです。
