...
- **一時的な失敗はライブラリが再試行します**（指数バックオフ・既定 3 回）。ただし **`create` は結果が不明でも再送しません**
  （非冪等で、消せないため）。そのときのエラーは **`retryable: false`** で、`hint` に「登録された可能性がある」ことが書かれます。
...
try {
  ...
  switch (e.category) {
    case "auth":
      // トークンを取り直せない → 権限付与（初回のブラウザ code 付与）と App ID / App Secret を確かめる
      break;
    case "permission":
...
- **トークン期限切れ → 自動リフレッシュ＋再試行**: リソース `401` / `402`
  （Access Token 期限切れ）は内部で Refresh して 1 回だけ自動再試行します。既定の取り方では、
  Refresh Token が失効・無効になっていても `code_direct` で取り直します。`PortersAuthError`
  （`category: "auth"`）が返るのは**`code_direct` でも取り直せないとき**（初回の権限付与が済んでいない・取り消された、
  App ID / App Secret が違う、など）です。
- **一時エラー・ネットワーク → 指数バックオフで自動リトライ**: リソース `9` / `302`（`transient`）と
  接続エラー（`network`）は再試行します。ただし**非冪等な `create`** は、送った後の接続エラーと `302` では
  登録まで進んだかが分からないので、二重登録を避けるため再試行せず、隠さずにエラーとして返します。
  このエラーは `retryable: false` で、`hint` に「登録された可能性がある」ことが書かれ、元のエラーは `cause` にあります。
  登録されたかを確かめてから、登録されていなければ作り直してください。`9` は処理されていないと分かるので、`create` でも再試行します。
...
| category     | 意味                                       | 主な原因 / 対処                                                              |
| ------------ | ------------------------------------------ | ---------------------------------------------------------------------------- |
| `auth`       | 認証情報・トークン・コード                 | 取り直せない → 権限付与（ブラウザ `code`）／資格情報を確認                   |
...
| `401`                                         | Refresh Token 期限切れ（既定の取り方は `code_direct` で取り直す） | `auth`       |
...
送った後の `create` に `302` が返ったときは、上の表と違い `retryable: false` で届きます（登録まで進んだかが分からないため）。
