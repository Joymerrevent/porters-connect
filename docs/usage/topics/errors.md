...
失敗したときに、落とすか・続けるか・再送するかを決めるためのページです。エラーの型と `category`、例外の
...
ダウンロードが終わるまでを数えます（大きな添付で効いてきます — [上限とレート][limits]）。
...
一括書き込みの単一レコード超過）は**従来どおり送信前に働き**、無駄な呼び出しは起きません。
変わったのは**例外の届き方だけ**です。
...
- **リクエストサイズ → 送信前の検査**: 全体 ~15000 文字を超える要求は、サーバの不透明な 400 を
  待たずに送信前へ `PortersConfigError`（`category: "config"`）で弾きます（`hint` に分割を提案）。

## 非 PORTERS ボディの HTTP エラー
...
この判定は**すべての経路で同じ**です — Resource API だけでなく、**OAuth / Token のやり取り**にも同じように効きます<!-- 根拠: ADR-0050 -->。
...
> 分からない」状態なので、**非冪等な `create` は自動再送しません**（`create` を再送しない規則がそのまま効く＝安全側）。
>
> なお**実 PORTERS がどの status を返すかは未確認**です（契約後に確認します）<!-- 根拠: LV-9 -->。
...
ライブラリは Read 応答を**ルート要素名と `<Code>` の両方**で見分けます。reference の成功形
...
new PortersClient({ hostname: "xxxxx.example.com/gw" }); // ❌ パス prefix は対象外
new PortersClient({ hostname: "a.test", port: 0 }); // ❌ port は 1〜65535 の整数
```

**ポートを `hostname` に書いても黙って落とされません。** そのまま通すと「指定したつもりで
...
`tenant(id, { fields })` を指します<!-- 根拠: ADR-0087 -->。黙って無視すると宣言が丸ごと捨てられ、
...
| `unknown`    | 未知（安全側の受け皿）                     | `code` と `hint` を確認／隠さず、エラーとして表に出している  |
...
`validation` のエラーとして表に出す（項目名付き。黙って別の値にはしない）」と決めており<!-- 根拠: ADR-0006 -->、いまは実装がそれに従います。
...
落ちます（`Link` のスカラ形＝Contact の ID も同じ）<!-- 根拠: RV-58 -->:
...
手前で厳しくすると、サーバーが受け付ける値をライブラリが落としてしまいます<!-- 根拠: RV-36 #4 -->。
...
`f.date()` と宣言した）。スカラどうしのずれが落ちるのは、変換を伴う日時と数値の経路だけです。
...
- 設計判断: ADR-0006（エラーモデル） ／ ADR-0044（HTTP ステータスの扱い） ／
  ADR-0046（例外の届き方） ／ ADR-0051（Read 応答の同定）
