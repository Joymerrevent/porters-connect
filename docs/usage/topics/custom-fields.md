...
**生成は開発時に行う作業で、アプリの実行時には要りません。** 型はコンパイル時に宣言を読んで付くので、
生成した宣言はファイルとしてプロジェクトに置き、アプリはそれを `import` して使います。生成する処理と、
宣言を使って読み書きする処理は、次のように分けます。

1. 開発時に、生成用のスクリプトで `generateFieldDecls` を呼び、返ってきたソースをファイルに書き出して
   リポジトリにコミットします（下のコード）。
2. アプリは、そのファイルの `myFields` を `porters.tenant(id, { fields: myFields })` に渡します。
   実行時に `generateFieldDecls` を呼んでも、返るのは文字列なので、その場で型には反映されません。
3. テナント側で項目が変わったら、1 をやり直して差分をコミットします。ずれているかどうかは、
   後述の `verifyFields` で確かめられます。
...
`src` は TypeScript のソースコードの文字列で、ライブラリはファイルを書きません。上の例で書き出した
`src/porters-fields.ts` の中身は、こうなります:
...
できた `myFields` は、アプリ側でこのファイルを `import` して、そのテナントのスコープに渡します — `porters.tenant(1, { fields: myFields })`。
