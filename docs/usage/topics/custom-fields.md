...
宣言のしかた、宣言を自動生成する方法、宣言がテナントの実物と合っているかを確かめる方法が分かります。
...
テキストの項目を `f.number()` や `f.date()` と宣言した場合も落ちます — 数値・日時に**読めない値**は
変換できないためです（`Link` のスカラ形＝Contact の ID も同じ）<!-- 根拠: RV-36・RV-58 -->。

**落ちないずれ方もあります。** 形が同じスカラどうしで変換を伴わない組み合わせ（実物 `Number` を
...
| 区分           | 意味                                     | 深刻度                                         |
| -------------- | ---------------------------------------- | ---------------------------------------------- |
| `typeMismatch` | 実在するが Data Type が違う              | **最悪**（読み取りが落ちるか型が違う値が入る） |
