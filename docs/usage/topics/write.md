...
**id は 1 以上の整数です。** PORTERS では書き込みの id `-1` が「新規作成」を意味するので、`update(-1, …)` を送ると
更新ではなくレコードが作られてしまいます。`0`・負の数・小数・`NaN` を渡すと、送る前に `PortersConfigError` になります
（`updateMany` も同じで、1 件でも当てはまると何も送りません）。
...
- `createMany` の件ごとの `code` が `302`（トランザクションエラー）のときは、そのレコードが作られたかが分かりません。再送する前に、あるかを確かめてください。
...
- 最初のバッチが「書き込まれていないと分かる」失敗（PORTERS が入力や権限の誤りで断ったとき、トークンの取得に失敗して何も送っていないときなど）なら、元のエラーをそのまま
  ...
  `createMany` を全体で再実行すると、成功済みのバッチの分がもう一度作られます（**非冪等**）。回復は、`hint` にある
  「書き込まれなかったレコード」だけを再送してください。書き込まれた可能性があるバッチは、あるかを確かめてから
  再送します（`updateMany` は id 指定で冪等なので、そのまま再送できます）。

- 応答の件数が送った件数と合わないときも、そのバッチは結果が分からないものとして同じ形で throw します。
