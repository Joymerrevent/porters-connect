...
他の型はそのまま送って PORTERS に判断させます。**手前で厳しくしすぎると、サーバーが受け付ける値を
ライブラリが落としてしまう**からです。読みと書きで検査の厳しさが違うのは、意図したものです。型ごとの値のかたちは
...
- **型が緩い側に倒してあります。** 利用側の誤りなら、1 回の呼び出しで型付きのエラーが返るだけで済みますが、型で弾かれると利用者の側に
  回避手段がありません。
