...
データ系リソースの書き込みは、単件の `create` / `update` と、まとめて書く `createMany` / `updateMany` の 4 つです。
このページは 1 件の書き方、読みと書きで形が違う項目、必須の規則、一括の分割と部分成功をまとめます。
...
ライブラリが落としてしまう**からです。この非対称は意図したものです。型ごとの値の形は
[項目と値の形][fields]にあります。
...
- 主題: [検索][query]／[項目と値の形][fields]（型ごとの読みと書きの形）／[エラーと再試行][errors]（部分成功と、再送してよいか）／
