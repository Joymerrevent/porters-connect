...
- **消せない前提で書きます。** 二重に作らない工夫が要り、`create` は自動で再送されず、テストデータも残ります。
...
消す必要があるときは PORTERS の画面で操作します。PORTERS Connect API の役目ではありません。
...
> 別の意思表示です。ライブラリは `"existing"` をそのまま送るので、**PORTERS が将来この既定を変えても
...
- `condition` に使えるのは **`P_Id` / `P_UpdateDate` / `P_UpdatedBy` の 3 つだけ**です。他の項目を指定すると
  `PortersConfigError`（hint 付き）になります。生存データが混ざる `"all"` でも同じです。PORTERS に送れば 400 になるので、
  ライブラリが送信前に弾きます。
...
- **Opportunity（案件）には `P_Deleted` がありません**（PORTERS が公表している項目一覧に無い）。`"all"` で読むと
  生存と削除済みを見分けられないので、`"existing"` と `"deleted"` を別々に読んでください。
...
- **差分取得で「消えたレコード」は追えません。** 削除は `itemstate: "deleted"` で別に読みます
  （[毎日の差分同期][sync-batch]）。
- **テスト環境のデータも消せません。** 契約なしで動かす仕組み（[契約なしでテストする][testing]）を用意している理由の
  1 つでもあります。

## 関連

- ガイド: [検索][query]（クエリの他の要素）／[書き込み][write]（`create` は再送されない）／[エラーと再試行][errors]
- リソース: [Process][r-process]（JOB × レジュメで一意。重複は `301`）
