...
- **alias は 3 種類**です。標準項目 `P_` の型はライブラリがあらかじめ持っていて、カスタム項目 `U_` / `A_` は `defineFields` で宣言すると型が付きます。
...
PORTERS が Data Type を与えていない項目があります。PORTERS のリファレンスで `ー` と書かれているもので、
...
`PortersResourceError`（`category: "validation"`）になります（入れ子で来るはずの値が単一で来るといった形の違い、または日時・数値に
