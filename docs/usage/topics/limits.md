...
方針は 1 つです — **送る前の検査を厳しくしすぎない**。
サーバーが受け付けるものをライブラリが弾くと、利用者には**回避手段がありません**。
サーバーが返すエラーなら型付きの [`PortersError`][error-handling] として受け取り、処置を選べます。
安全側に倒れるのは、サーバーに判断させるほうです。
...
- 選択肢 alias を**外から受け取る**なら（フォーム・取り込み・ツール引数）、
  止められるのはこの検査だけです。検査が無いと、値が XML の構造として解釈され、
  **意図しないレコードが書き換わります**。
...
どうやってもその中に収まらないためです。代わりに、送信前に次の 3 つを検査します<!-- 根拠: ADR-0064 -->。
...
（空要素を送ると消えるのか、PORTERS のリファレンスに記載が無いため）<!-- 根拠: LV-22 -->。
項目を省略すれば値は変わりません。

## レート — 待つもの、数えないもの

上限は 2 つあり、**ライブラリが守るのは分あたりだけ**です。

| 上限                           | 誰が見るか                                     |
| ------------------------------ | ---------------------------------------------- |
| 1 分 **Read 2000 / Write 500** | **ライブラリ**（内蔵スロットリングが守る）     |
| 月 **約 15 万アクセス**        | **利用側**（契約条件。ライブラリは数えません） |

### 分あたりは、待って収める

直近 60 秒に送った時刻を覚えて、上限内に抑えます<!-- 根拠: ADR-0010 / ADR-0102 -->。既定では上限の **90%**（`safety: 0.9`。下記「指定できる値」）までしか使いません
（Read 1800 / Write 450 相当）。使い切ったら**待ちます** — 例外にはしません。
**どの 60 秒を切り取っても**その数を超えません。最初の 1800 件はすぐに送り、次の 1 件は最初の 1 件から 60 秒たったときに送ります。

**数えているのは接続先（ホストとポートの組）1 つ分です**<!-- 根拠: ADR-0073 -->。クライアントの数ではありません。
トークンを分けるためにクライアントを分けても（[複数テナント][multi-tenant]の「認証を分けるか」）、同じ接続先を
向いている限り**バケットは 1 つ**で、合計が上限に収まります。
ローカルで動かすフェイクサーバー（[契約なしでテストする][testing]）は別ホストなので、本番向けの枠を使いません。
...
（`take()` が解決しない実装）。上限 0 を受け付けて待ち続けると、
設定の誤りに気づけないので避けています。
...
月あたりのアクセス数は**契約条件**であって、PORTERS の API から取得できる値ではありません。プロセスを
跨いだ累積をライブラリが正しく数えることはできない（再起動・複数インスタンス・別アプリ）ので、
**利用側の運用責務**にしてあります。

**認証のリクエストも 1 アクセスとして数えられます。** トークンを永続化する（`tokenStore`）と
取り直しが減ります（[認証とトークン][authenticate]）。差分取得やキャッシュも同じようにアクセス数を減らします。
...
ダウンロードが遅ければそこで打ち切られます**（`AbortSignal` が fetch 全体に適用される）。
中断は `PortersNetworkError`（`category: "network"`・`retryable: true`）になるので、
[エラーと再試行][failures]の分岐はそのまま使えます。

**数え方は 1 リクエストごと**です。自動リトライは既定で最大 3 回なので、通らないリクエストは
最悪で 4 回 × 30 秒に、バックオフの待ち時間を加えた時間がかかります（トークンの失効で取り直したときの 1 回と、
トークンを取りに行くリクエストは、この外で別に数えます）。**レートの待ち時間は含みません**（スロットルは
タイムアウトの外側で、実際に送る直前からタイマーが回ります）。

**影響が出るのは大きな添付です。** 添付の本体は最大 10MB（[添付ファイル][attachments]）で、
細い回線では 30 秒に収まらないことがあります。**延ばせます** — `transport` オプションに `createFetchTransport({ timeoutMs })` を
渡してください。
...
`timeoutMs` は**正の整数（ミリ秒）**で、**2,147,483,647 まで**（約 24.8 日。Node のタイマーの上限）を受けます。
`0` は「無制限」ではなく**即中断**なので、構築時に `PortersConfigError` で弾かれます。上限を超える値も、
すべてのリクエストがすぐ中断されるので、同じく弾かれます。短くすることもできます（対話的なツールで待たせたく
ないときなど）。
