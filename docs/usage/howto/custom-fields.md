...
|            | 宣言しない                                                                                                             | 宣言する                                                                              |
| ---------- | ---------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| 型         | 結果の型に出ない（`as` で受ける）                                                                                      | `Candidate` / `SearchQuery` に現れる                                                  |
| 値         | PORTERS が返した生の文字列のまま（日時なら `2026/09/10 12:00:00`。入れ子で返る `Option` / `User` / `Image` は `null`） | Data Type どおり（`Number` → `number`、`DateTime` → ISO 8601、`Option` → `string[]`） |
| 既定 field | 送られない＝`field` に書かない限り取得されない                                                                         | `field` 省略時に**自動で要求される**（[ADR-0020][adr20]）                             |

### 書き込み

|            | 宣言しない                                                         | 宣言する                                                                                     |
| ---------- | ------------------------------------------------------------------ | -------------------------------------------------------------------------------------------- |
| 型         | 入力の型に無い（cast が要る）                                      | `CreateInput` / `UpdateInput` に現れる                                                       |
| 値         | `String(値)` にして素通し。ISO 8601 の日時も配列もそのまま送られる | Data Type どおりに組み立てる（ISO 8601 → PORTERS 形式、`Option` → 子要素、`Image` → 3 要素） |
| 既定 field | **該当なし**（渡した項目だけが送られる）                           | 該当なし（同左）                                                                             |
