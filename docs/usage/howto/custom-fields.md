...
|              | 宣言しない                                                                                                                                                                                                                                | 宣言する                                                                                                                                                                                                           |
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **読み取り** | **型**: 結果の型に出ない（`as` で受ける）／**値**: PORTERS が返した生の文字列のまま（日時なら `2026/09/10 12:00:00`。入れ子で返る `Option` / `User` / `Image` は `null`）／**既定 field**: 送られない＝`field` に書かない限り取得されない | **型**: `Candidate` / `SearchQuery` に現れる／**値**: Data Type どおり（`Number` → `number`、`DateTime` → ISO 8601、`Option` → `string[]`）／**既定 field**: `field` 省略時に自動で要求される（[ADR-0020][adr20]） |
| **書き込み** | **型**: 入力の型に無い（cast が要る）／**値**: `String(値)` にして素通し。ISO 8601 の日時も配列もそのまま送られる／**既定 field**: **該当なし**（渡した項目だけが送られる）                                                               | **型**: `CreateInput` / `UpdateInput` に現れる／**値**: Data Type どおりに組み立てる（ISO 8601 → PORTERS 形式、`Option` → 子要素、`Image` → 3 要素）／**既定 field**: 同じく該当なし                               |

**既定 field が効くのは読み取りだけ**です。書き込みは渡した項目だけが送られるので、
宣言しても送られる項目は変わりません。読み取りは逆で、宣言していない項目は
`field` に書かない限り**要求すらされません**。

型の出かたが読み書きで違うのは、**要求と受け取りが別だから**です。`field` は `U_` / `A_` で
始まる名前なら未宣言でも書けます（[ADR-0059][adr59]）が、返ってきた値には型が付きません。
`condition` / `order` と書き込みは**カタログ済みの alias しか受けない**ので、未宣言のまま使うには
cast が要ります。
