...
| 既定 field | **返ってこない。** `field: ["U_score"]` と書いて初めて返る                                                                                   | **返ってくる。** `field` を省略しても要求される（[ADR-0020][adr20]）                |
...
**書き込みに「既定 field」の行が無いのは、その仕組みが無いから**です。読み取りは `field` を
省略できる（＝省略したとき何を要求するかが問題になる）のに対し、書き込みは
`create` / `update` に書いた項目だけが送られます。宣言してもここは変わりません。
