...
このページでは、Company DB を指定し、条件で探し、1 件を取り、全件を回します。読み取りは 3 つ覚えれば足ります。
**どの Partition か**を指定して、**どの項目が欲しいか**を書いて、`search` か `get` を呼ぶ。終わると、返ってくる値の
かたちも分かります。
