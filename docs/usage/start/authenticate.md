...
| **以降の運用**     | 手元にトークンが無いとき（期限が近いものは Refresh Token で更新する） | ライブラリ（自動）   | `code_direct`（ブラウザ不要） |
...
| 症状                                                                  | よくある原因                                                                                                        |
| --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| 構築した瞬間にエラーになる                                            | `hostname` に `https://` ・パス・ポートが入っている（[前ページ][s-install]）／`tokenProvider` に `acquire()` が無い |
