...
- **参照 6 項目は条件付き必須**（出典の `※`）で、型では止めません。規則は、項目どうしの依存関係です。
  ...
  - 規則から外れると PORTERS が Result Code で返します（`create` は `P_Owner` だけを必須にしてあり、`P_Client` だけの正当な呼び出しを弾かないため）。
...
| `P_Client`    | ※（条件付き必須） | 型は止めない。PORTERS が判定し、外れると Result Code で返る（条件は「固有の注意」） |
| `P_Recruiter` | ※（条件付き必須） | 型は止めない。PORTERS が判定し、外れると Result Code で返る（条件は「固有の注意」） |
| `P_Job`       | ※（条件付き必須） | 型は止めない。PORTERS が判定し、外れると Result Code で返る（条件は「固有の注意」） |
| `P_Contract`  | ※（条件付き必須） | 型は止めない。PORTERS が判定し、外れると Result Code で返る（条件は「固有の注意」） |
| `P_Candidate` | ※（条件付き必須） | 型は止めない。PORTERS が判定し、外れると Result Code で返る（条件は「固有の注意」） |
| `P_Resume`    | ※（条件付き必須） | 型は止めない。PORTERS が判定し、外れると Result Code で返る（条件は「固有の注意」） |
