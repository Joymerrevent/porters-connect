...
- **`P_Required` は、テナントが入力必須にした項目です。** `generateFieldDecls` はこれを見て、カスタム項目の宣言に `{ required: true }` を付けます（[カスタム項目][custom-fields]）。
...
- ガイド: [カスタム項目][custom-fields]（宣言の生成と突き合わせ）／[日時と時分型][datetime]（時分型の見分け）
