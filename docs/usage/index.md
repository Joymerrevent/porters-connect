...
**順に読む章です。** 契約済みの状態から、自分のアプリを PORTERS に繋ぎ、読み書きして、本番に出すまでの
順路で、6 ページあります。
...
**認証・検索・書き込み・カスタム項目・上限といった主題ごとの規則と、その背景にある PORTERS 側の前提を
理解する章です。** 1 主題 1 ページで、各ページは **まず知ること（PORTERS 側の前提）→ 使い方 → 細かい規則** の順で
...
**Candidate や Job など 18 のリソースで、何が呼べて何に気をつけるかを引く章です。** 1 リソース 1 ページで、`porters.tenant(id)` で束ねたスコープ（`t`）の下にある 18 リソースを、
...
**毎日の差分同期や複数テナントのような用途を、ライブラリのどの機能で組み立てるかを知る章です。** 各ページは **使う機能 → 組み立て → ライブラリの外（利用側の責務）** の
...
**公開 API の型・関数と、PORTERS API の事実を引く章です。**
