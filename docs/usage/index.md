...
- XML の応答を型付きオブジェクトに変換し、入力もふつうの JS の値で受けます。XML は外に出しません。
- 独自 OAuth を意識せずに使えるようにします。初回の権限付与のあとは、トークンの取得・更新を無人で行います。
- 1 分あたりのレート上限を内蔵スロットルで守り、一時的な失敗は再試行します。
...
- 削除は PORTERS の画面で行います。Connect API に削除が無いので、`delete()` はありません。
...
`porters`（PortersClient）・`porters.auth`・`tenant(id)` のスコープ・単独の関数について、何が呼べて、何を渡し、
何に気をつけるかを調べる章です。読者の順路（`porters` → `tenant(id)` → `t.candidate`）のうち、リソースの手前に
あるものを 1 ページずつ引けます。構築オプションや `auth` のメソッド一覧を確かめたいときに開いてください。
...
- [単独の関数][cl-functions] — 宣言と突合・上限と接続・値の変換の関数を用途別に
...
| `defineFields` や `createThrottle` などの関数を引きたい | [単独の関数][cl-functions]                                               |
...
[cl-functions]: client/functions.md
[api]: api/index.md
[auth]: topics/auth.md
