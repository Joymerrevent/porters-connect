...
// 導入と、引く 5 章（ガイド・クライアント・リソース・関数・実践例）。リファレンス（`reference/` と生成物）は探す先に数えない。
const GUIDE_GLOBS = [
  "docs/usage/start/**/*.md",
...
console.error(
  `\n導入（docs/usage/start）か、ガイド・クライアント・リソース・関数・実践例（docs/usage/{topics,client,resources,functions,recipes}）の\n` +
    `どこかで使い方に触れてください。\n` +
