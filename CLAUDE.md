...
- 哲学：「動くものではなく、動き続けるものを作る」→ 薄く・堅く・メンテしやすく
...
3. **UTC 前提**。日本時間運用では +9h 補正が必要。`util/datetime.ts` に集約。
...
MVP 優先順：**OAuth → Candidate → Job → Client → Process → Resume**。残りは v0.2 以降。
...
- テストを伴わない新リソース追加はしない。
- コメント・JSDoc・型名は**英語**（海外利用者を想定）。READMEは英語メイン＋ `README.ja.md` 併記。
