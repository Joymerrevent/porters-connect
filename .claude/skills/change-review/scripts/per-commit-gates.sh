...
#   - **`check:api` はここで回せない**（偽の赤になる）。worktree では typedoc がリモート URL を
#     解決できず、生成物のソースリンクが `[src/x.ts:71](https://github.com/…)` から
#     `src/x.ts:71` に変わる。実測（2026-09-12）で 175 ファイル・437 行が丸ごと差分になり、
#     全コミットが赤と表示された。**本物のチェックアウト（CI と手元）では通る**。
