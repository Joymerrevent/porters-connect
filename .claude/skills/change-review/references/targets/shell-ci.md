...
- **実装が信じている不変条件を破るモードを 1 つ以上置く。** 偽物を実装と整合的に作ると、
  **実装が既に扱える経路しか踏めない**。repo に存在しない head SHA、判定表に無い PR 番号、
  桁数の違う指紋 — 「そんな値は来ない」と実装が仮定しているものを返させる。
  実例: 偽 gh が返す head を必ず使い捨て repo に存在させていたため、「head を解決できないのに
  **force-push と報告する**」経路を 2 ラウンド見逃した。最終的に見つけたのは、意図した試験では
  なく**ハーネス自身のバグが空の head を渡した事故**だった。整合的な偽物は事故ほどにも働かない。
