...
- **`references/report-format.md` を読んでから書く**。台帳の ID は不変・行は消さない・状態を更新する。
...
- 既存 open 指摘が解消されていれば `fixed` にして履歴に直り方を残す。仕様として許容なら `wontfix`、契約待ちは `deferred`。

## やらないこと

- **勝手にコードを修正しない**。このスキルは診断と記録まで。修正は別途ユーザーの指示で。
- 既存の `findings.md` 行を削除・上書きしない（履歴の破壊はフェイルセーフに反する）。
