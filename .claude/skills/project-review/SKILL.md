...
4. **レポート出力** — `references/report-format.md` の様式で:
   ...
   - `docs/reviews/YYYY-MM-DD-N.md`（その回のスナップショット）を**新規作成**。`N` はその日の run 連番（1 始まり）。**同日に複数回回しても既存スナップショットへ追記せず、連番で別ファイルを切る**（スナップショットは点景＝後から書き換えない）。同日の前 run があればそれ（`-1`）から相互にリンクする。
