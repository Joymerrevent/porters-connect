...
description: >-
  ...
  されなくても必ずこのスキルを使う。品質ゲート実行（scripts/gates.sh で一括）と、
  API 忠実性・ADR 遵守・型安全・エラーモデル・テスト厳密性・ドキュメント・公開準備の
  観点別レビューを束ね、findings 台帳と日付スナップショットを更新し、その成果物を
  ブランチにコミットするところまで行う。
---
...
4. **レポート出力** — `references/report-format.md` の様式で:
   - **台帳を更新する**（[ADR-0052][adr52]・1 件 1 ファイル）:
     - 新規指摘は **雛形 `docs/reviews/rv/0000-template.md` をコピー**して
       ...
       **`影響`（なぜその重要度か）を必ず書く** — ここが薄いと後から重要度を検証できない。
     - 既存 open は**そのファイルの「状態」と「処置」を更新**（ファイルは消さない）。
     - **索引 `docs/reviews/findings.md` にも必ず 1 行**足す／直す。**`pnpm check:index` を通してから終える**
       （索引と本文のズレは CI で落ちる）。
   - `docs/reviews/YYYY-MM-DD-NN.md`（その回のスナップショット）を**新規作成**。`NN` はその日の run 連番（**2 桁ゼロ埋め**・`01` 始まり）。**同日に複数回回しても既存スナップショットへ追記せず、連番で別ファイルを切る**（スナップショットは点景＝後から書き換えない）。同日の前 run があればそれ（`-01`）から相互にリンクする。
   - 初回のみ `docs/reviews/README.md` を生成。
   日付は環境のシステム日付（currentDate）を使う。`references/report-format.md` を読んでから書くこと。
5. **コミットまで終える** — レビューは記録がリポジトリに載って初めて完了（ユーザー指示・2026-09-21）。
   `develop` は保護されているので、**ブランチを切ってコミットする**（`docs/project-review-YYYY-MM-DD`）。
   - 対象は**そのレビューで生成・更新したファイルだけ**（スナップショット・`findings.md`・新設／更新した
     `rv/*.md`・初回なら `README.md`）。**コードやドキュメント本体は含めない**（診断と修正は別 PR）。
   - 件名は過去の形に揃える: `docs: YYYY-MM-DD のプロジェクトレビューを記録する（RV-N〜M）`
     （commitlint: 先頭小文字・ゲート結果と新規指摘を本文に要約）。
   - コミット前に `pnpm check:index` / `check:links` / `lint:md` / `format:md:check` を通しておく
     （pre-commit の lint-staged が整形するので、通していないと差分がコミットとズレる）。
   - push と PR 作成は `permissions.ask` の確認対象。**マージはユーザーが行う**（ユーザー規約）。
6. **要約を会話に返す** — ヘッドライン指摘（最も重要な 1〜2 件）、ゲート結果、新規/状態変化した指摘 ID、次アクション候補、コミットしたブランチ名を簡潔に。詳細はレポートに書いたと伝える。
...
- スナップショットの観点別所見は **「なぜ」を残す**（台帳の要約コピーにしない）。退行監視のため**良かった点**も書く。
  **指摘の詳細（概要・根拠・推奨・処置）は `rv/NNNN-*.md` が正**なので書き写さず、ID とリンクで指す。
  スナップショットに残すのは「その回に何を見つけたか」と「そこでしか分からないこと」（気づいた経緯・判断の背景・
  その回だけの調査結果）。
- 既存 open 指摘が解消されていれば `状態` を `fixed` にし、`処置` に何をしたかを記入する。仕様として許容なら `wontfix`、契約待ちは `deferred`。
  **本体（`rv/*.md`）と索引（`findings.md`）の両方**を直す — 片方だけだと `pnpm check:index` が落ちる。
  `fixed` にするときは **`検証` 節に「直ったと言い切れる根拠」**（どのテストが何を pin しているか）を残す。

## やらないこと

- **勝手にコードを修正しない**。このスキルは診断と記録（＝レビュー成果物のコミット）まで。修正は別途ユーザーの指示で。
- 既存の `docs/reviews/rv/*.md` を削除・上書きしない（判断経緯の消失はフェイルセーフに反する）。
  状態が変わっても**ファイルは残し、「状態」と「処置」だけを更新**する。
