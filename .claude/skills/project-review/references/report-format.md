...
└── YYYY-MM-DD-N.md      # その run のスナップショット（N=その日の連番・同日も追記せず別ファイル）
```
...
## 2. `docs/reviews/YYYY-MM-DD-N.md`（スナップショット）

その run 一回限りの記録。台帳が「現在地」なら、こちらは「その時の写真」。**ファイル名は必ず連番付き**（`N` はその日の run 番号・1 始まり）。**同日に複数回 review しても既存ファイルへ追記せず、新しい連番ファイルを切る**（写真は後から書き換えない＝差分・履歴が追える）。同日の前 run があれば相互リンクする。
