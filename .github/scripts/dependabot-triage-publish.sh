...
template_headings=$(grep -E '^## ' "$TEMPLATE" 2> /dev/null | sed 's/[[:blank:]]*$//')
if [ -z "$template_headings" ]; then
  fail "雛形 ${TEMPLATE} から見出しを読めません（移動・改名された可能性）。構造を検証できないので投稿しません"
  problems=1
else
  # 行全体で一致させる（`-x`）。部分一致だと `## 判定表` が `### 判定表` にも当たり、
  # 見出しレベルを変えたレポートが「所定の形式です」で通る。コードブロックの中に
  # 見出し文字列があるだけでも通ってしまう。雛形は「見出しを足さない・消さない・
  # 並べ替えない」を要求しているので、せめてレベルは機械で守る。
  #
  # ただし**末尾の空白だけで落とすのはやりすぎ**。見出しの構造は壊れていないのに
  # その日のレポートが出なくなる＝厳しすぎる安全装置は外される側に回る。両側から
  # 末尾の空白とタブだけを落としてから比べる（CR は落とさない。CRLF は形式の破損
  # そのものなので、下の機械可読行の検証で理由付きで落ちる方がよい）。
  report_headings=$(sed 's/[[:blank:]]*$//' "$REPORT")
  while IFS= read -r heading; do
    printf '%s\n' "$report_headings" | grep -qxF "$heading" || {
