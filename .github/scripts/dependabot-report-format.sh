...
# ここが受け持つのは「その中で機械が値を取り出す 3 行」— その形（正規表現）と、
# 値の解釈（to_epoch）の両方。読む側と検証側で解釈が割れると、形が合っているのに
# 意味が食い違う状態になるので、解釈もここに置く。
# この 3 行を変えるときは、雛形とワークフローのプロンプトも同時に直すこと。
#
# このファイルは定義だけを持ち、値を使うのは source する側（gate / publish / merge）。
# 静的解析からは「代入したのに使っていない」に見えるので、ファイル単位で黙らせる
# （行頭を `# shellcheck` で始めるとディレクティブとして解釈されるので書き方に注意）。
