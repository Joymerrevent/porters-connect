...
# 永久に取りこぼす形で静かに壊れる）。
#
# ただし**判断材料そのものが取れない**場合はこの原則の外で、走らせずに job を落として
# 知らせる。「材料はあるが前回との比較が分からない」と「材料が無い」は違う状態だからで、
# 前者を抑止に倒すと静かに壊れるが、後者を実行に倒しても事実ゼロでは判定できず、
# 中身の無いレポートが出るだけになる。どちらも安全側は「気づける方」に倒すことで、
# 前者はそれが実行、後者はそれが失敗の通知になる。
set -uo pipefail
