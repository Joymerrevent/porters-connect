...
| 章               | 何が書いてあるか                                                                                                                                                     |
| ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **導入**         | 順に読む 6 ページ。PORTERS 側の準備、インストール、認証と疎通の確認、はじめての読み取りと書き込み、本番に出す前の確認                                                |
| **主題別**       | Partition・項目と値・検索・書き込み・削除・日時・カスタム項目・認証・上限・エラー・テストの 11 の主題を 1 ページずつ。PORTERS 側の前提と、ライブラリがどこまで行うか |
| **クライアント** | `PortersClient` の構築オプション、`auth` の 6 メソッド、`tenant(id)` のスコープから使えるもの                                                                        |
| **リソース別**   | マスタ系 5 種とデータ系 13 種を 1 ページずつ。呼べるメソッド・固有の注意・新規作成の必須項目・項目と型                                                               |
| **関数**         | `import` して呼ぶ関数。カスタム項目の宣言と突き合わせ、上限と接続の差し替え、値の変換                                                                                |
| **実践例**       | 毎日の差分同期・複数テナント・トークンの DB への保存・中央のサービスからのトークンの受け取り。機能の組み合わせ方を通しで示す                                         |
...
PORTERS 側の仕様で、使い始める前に知っておきたいものです。詳しくは、それぞれの主題のページの「まず知ること」に
あります。
