...
| **ガイド**       | Partition・項目と値・検索・書き込み・削除・日時・カスタム項目・認証・上限・エラー・テストの 11 の主題を 1 ページずつ。PORTERS 側の前提と、ライブラリがどこまで行うか |
| **クライアント** | `PortersClient` の構築オプション、`auth` の 6 メソッド、`tenant(id)` のスコープから使えるもの                                                                        |
| **リソース**     | マスタ系 5 種とデータ系 13 種を 1 ページずつ。呼べるメソッド・固有の注意・新規作成の必須項目・項目と型                                                               |
