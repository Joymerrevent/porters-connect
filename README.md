...
| 章               | 概要                                                                                                                                                                 |
| ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **導入**         | 自分のアプリを PORTERS に接続し、データを読み書きして、本番で動かせるようになるまでを 6 ページで順に進めます                                                         |
| **ガイド**       | PORTERS を扱ううえで避けて通れない主題（Partition・検索・書き込み・認証・上限など）を 1 つずつ理解します。PORTERS 側の前提と、ライブラリがどこまで行うかが分かります |
| **クライアント** | クライアントの作り方と、そこから呼べるものを確かめるときに開きます                                                                                                   |
| **リソース**     | 18 のリソースそれぞれについて、何が呼べて、何に気をつけ、新規作成で何が必須かを確かめるときに開きます                                                                |
| **関数**         | `import` して呼ぶ関数の使い方と、失敗したときの届き方を確かめるときに開きます                                                                                        |
| **実践例**       | 毎日の差分同期や複数テナントなど、実務でよくある用途を題材に、ライブラリの機能の組み合わせ方を通して示します                                                         |
| **リファレンス** | 型やメソッドの正確な定義（[公開 API リファレンス][api-ref]）と、PORTERS 側の仕様（[PORTERS API の事実][ref]）を確かめるときに開きます                                |
