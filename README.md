...
- **上限内に自制する**：スロットリング・リトライ（指数バックオフ）・リクエストの長さの検査を内蔵。
...
**[docs/usage][docs-index] が目次**です。5 つの章に分かれていて、順に読むのは導入だけです。

| 章               | 何が書いてあるか                                                                            |
| ---------------- | ------------------------------------------------------------------------------------------- |
| **導入**         | 順に読む 6 ページ。準備 → 導入 → 認証と疎通 → 読み → 書き → 本番前                          |
...
- **削除 API が存在しない**。`delete()` は型の上でも生やしていません（[削除と削除済みデータ][c-no-delete]）。
...
- **PORTERS 製品 8.x / 9.x は参考**：v2 が提供される製品世代です（個別マイナーの動作保証はしません）。**正典は [PORTERS API の事実][ref]**（PORTERS の公式ドキュメントに基づく）。
