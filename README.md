...
| **実践例**       | 毎日の差分同期・複数テナント・トークンの保存など、用途に沿った組み立て                      |
...
この README で説明しているのは、インストールと、クライアントを作って 1 回検索するところ（クイックスタート）までです。
認証の準備・書き込み・エラーの扱いなど、使い方の全体は [docs/usage][docs-index] の目次から読めます<!-- 根拠: ADR-0070 -->。
