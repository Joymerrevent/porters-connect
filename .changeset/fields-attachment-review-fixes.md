...
- **添付ファイルの `create` / `update` が、書き込む値を送る前に確かめるようになりました。** `resourceId` は正の整数、`contentType` と `fileName` は空でない文字列、`content` は Base64 の文字だけです。これまでは、JavaScript から渡し忘れると `undefined` の文字列を送り、壊れた添付ファイルができていました（添付ファイルは削除できません）。
