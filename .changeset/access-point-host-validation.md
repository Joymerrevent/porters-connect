...
アクセスポイントの書式を構築時に検証（ADR-0048）: `host` はホスト（＋必要ならポート）だけを表し、スキーム・パス・userinfo・空白を含む値は
...
弾かれる例: `https://a.test` ／ `""` ／ `a.test/` ／ `a.test/gw` ／ `user@a.test` ／ `a test` ／ 非 ASCII ホスト（punycode で渡す）／
既定ポート付き `a.test:443`（`:443` は省く）。通る例: `a.test` ／ `a.test:8080` ／ `127.0.0.1:4010`。
