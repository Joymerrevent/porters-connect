...
- **`createMany` の最初のバッチで、トークンの取得に失敗して何も送らなかったときは、元のエラーをそのまま返します。** これまでは「書き込まれた可能性がある」エラーにしていました。2 つ目以降のバッチでは、そのバッチが書き込まれていないと案内します。
...
- 次の 3 つを文書に書きました。
  ...
  - `searchAll` は、辿っている途中でレコードが減ると取りこぼすことと、減りうるときの辿り方。
